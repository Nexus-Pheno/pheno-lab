...
function near(
  ...
  if (!ok) failures++;
  console.log(
    `  ${ok ? "✓" : "✗"} ${label}${ok ? ` = ${actual}` : `  expected ~${expected}, got ${actual}`}`,
  );
}
...
function crossCheck(s: JvScan, ffTol = 0.05, pceTol = 0.05) {
  ...
  if (voc && isc && pmax && ff !== undefined) {
    near(
      `    FF recomputed from Pmax/(Voc·Isc)`,
      (pmax / (voc * isc)) * 100,
      ff,
      ffTol,
    );
  ...
  if (pmax && area && pce !== undefined) {
    near(
      `    PCE recomputed from Pmax/(area·100)`,
      (pmax / (area * 100)) * 100,
      pce,
      pceTol,
    );
...
);
{
  ...
  check(
    "serials",
    r.scans.map((s) => s.serial),
    ["C1-1", "C1-2", "C2-1", "C2-2"],
  );
  check(
    "all reverse",
    r.scans.every((s) => s.direction === "REVERSE"),
    true,
  );
  check(
    "all 101 points",
    r.scans.every((s) => s.curve.length === 101),
    true,
  );
  check(
    "PCEs",
    r.scans.map((s) => s.metrics.pce),
    [25.804105, 25.498632, 25.324962, 25.463331],
  );
  check(
    "distinct timestamps",
    new Set(r.scans.map((s) => iso(s.measuredAt))).size,
    4,
  );
...
console.log("\n── LIGHTSKY (大太阳) · saved session, 3 traces ──");
{
  ...
  check(
    "serials (direction suffix stripped)",
    r.scans.map((s) => s.serial),
    ["06-8", "05-8", "10-8"],
  );
  check(
    "directions",
    r.scans.map((s) => s.direction),
  ...
  check(
    "sign was flipped to photocurrent-positive",
    s.settings.signFlipped,
    true,
  );
  near("I at first point is now positive", s.curve[0].i, 140.9973999999999);
  near(
    "measuredAt takes 15:29:40 from the summary, date from the folder",
    s.measuredAt!.getTime(),
    Date.parse("2026-08-20T07:29:40Z"),
  );
