...
async function clean() {
  ...
  const rigs = await prisma.instrument.deleteMany({
    where: { name: RIG, hostname: "MOCK-CHLOE-PC" },
  });
...
async function main() {
  ...
  const experiment = await prisma.experiment.create({
    data: {
      ...
      createdById: user.id,
      observation:
        "Four hole-transport variants processed side by side on the same day.",
      hypothesis:
        "The p-SAM variant (Pcell17) gives the highest PCE at 0.04 cm².",
      samples: { create: samples },
      characterizations: {
        create: [
          {
            position: 0,
            processId: jvProcess.id,
      ...
      ...(spin
        ...
        : {}),
      runs: {
        create: [{ runNo: 1, status: "IN_PROGRESS", technicianId: user.id }],
      },
    },
  ...
  console.log(`\nCreated ${code} — "${TITLE}"`);
  console.log(
    `  ${experiment.samples.length} samples in ${GROUPS.length} variation groups`,
  );
  console.log(
    `  each answers to both serials, e.g. ${seeded[0].code} → ${seeded[0].instrumentCodes.join("  +  ")}`,
  );
  ...
  for (const name of files) {
    ...
    form.set(
      "sourcePath",
      `D:\\IV Measurement System\\Data\\20260408\\${name}`,
    );
  ...
  for (const o of [
    ...new Set(orphans.map((m) => `${m.serial} — ${m.matchNote}`)),
  ].slice(0, 6)) {
    console.log(`  · ${o}`);
  }
  ...
  const results = await prisma.characterizationResult.findMany({
    ...
    include: {
      sample: {
        select: { code: true, variationGroup: true, instrumentCodes: true },
      },
    },
  });
  const rows = results
  ...
  for (const g of GROUPS) {
    ...
    console.log(
      `  ${g.group} ${g.label.padEnd(9)} n=${list.length}  mean ${mean.toFixed(2)}%  best ${Math.max(...list).toFixed(2)}%`,
    );
  }
