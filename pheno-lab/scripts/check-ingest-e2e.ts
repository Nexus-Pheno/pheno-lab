...
async function post(
  ...
  form.set(
    "sourcePath",
    `D:\\IV Measurement System\\Data\\20260820\\${fileName}`,
  );
...
async function rematchThroughApi(orgSlug: string): Promise<RematchSummary> {
  ...
  if (!secret)
    throw new Error(
      "INGEST_CRON_SECRET is required to exercise /api/ingest/rematch",
    );
  ...
  const body = (await response.json()) as {
    error?: string;
    results?: Record<string, RematchSummary>;
  };
  if (!response.ok || !body.results?.[orgSlug]) {
    throw new Error(
      `Rematch endpoint failed (${response.status}): ${body.error ?? "missing organization result"}`,
    );
  }
...
async function main() {
  ...
  try {
    console.log(`\n── auth ──`);
    const bad = await fetch(`${BASE}/api/ingest/heartbeat`, {
    ...
    const good = await fetch(`${BASE}/api/ingest/heartbeat`, {
      headers: { Authorization: `Bearer ${key}` },
    });
    ok(
      "the real key passes the test endpoint",
      good.status === 200,
      String(good.status),
    );
    ...
    const raw = readFileSync(
      path.join(FIXTURES, "giantforce-auto-single.csv"),
    ).toString("utf8");
    const pixel1 = Buffer.from(
      raw.replace("Serial NO.:,C1-1", `Serial NO.:,${CODE}-S1-1`),
      "utf8",
    );
    // Second pixel: same sample, deliberately WORSE (PCE 25.80 → 21.50).
    const pixel2 = Buffer.from(
      raw
        .replace("Serial NO.:,C1-1", `Serial NO.:,${CODE}-S1-2`)
        .replace("Efficiency(%):, 25.804105", "Efficiency(%):, 21.500000"),
      "utf8",
    );

    const r1 = await post(
      pixel1,
      `${CODE}-S1-1_Cindy_perovskite_Light_Normal_Rev_1_143040.csv`,
      key,
    );
    ok(
      "pixel 1 stored",
      r1.code === 200 && r1.body.status === "stored",
    ...
    const r2 = await post(
      pixel2,
      `${CODE}-S1-2_Cindy_perovskite_Light_Normal_Rev_1_143103.csv`,
      key,
    );
    ok(
      "pixel 2 stored",
      r2.code === 200 && r2.body.status === "stored",
      JSON.stringify(r2.body),
    );

    const dup = await post(pixel1, `copy-of-pixel1.csv`, key);
    ok(
      "re-sending identical bytes is a 409 duplicate",
      dup.code === 409,
      JSON.stringify(dup.body),
    );

    const measurements = await prisma.jvMeasurement.findMany({
      where: { instrumentId: rig.id },
      orderBy: { serial: "asc" },
    });
    ok(
    ...
    ok(
      "both matched to sample S1",
      measurements.every((m) => m.sampleId === s1.id),
    );
    ok(
      "pixel suffix noted, not modelled",
      /Pixel 1/.test(measurements[0].matchNote),
      measurements[0].matchNote,
    );
    ok(
      "run attached",
      measurements.every((m) => m.runId === exp.runs[0].id),
    );
    ok("curve stored", (measurements[0].curve as unknown[]).length === 101);

    const result = await prisma.characterizationResult.findFirst({
      where: { sampleId: s1.id },
    });
    ok("a sample-level result was auto-filled", !!result);
    ok(
      "it took the BEST pixel (25.80, not 21.50)",
      (result?.metrics as Record<string, string>)?.["PCE (%)"] === "25.80",
      JSON.stringify(result?.metrics),
    );
    // jsonb does not preserve key order, so compare as a set.
    const labels = Object.keys((result?.metrics ?? {}) as object).sort();
    ok(
      "metric labels match the capture portal's",
      JSON.stringify(labels) ===
        JSON.stringify(["FF (%)", "Jsc (mA/cm²)", "PCE (%)", "Voc (V)"]),
      JSON.stringify(labels),
    );
    ok("marked as instrument-sourced", result?.source === "INSTRUMENT");
    ok(
      "note explains the pick",
      /best of 2 scans/.test(result?.note ?? ""),
      result?.note,
    );
    ...
    const pixel3 = Buffer.from(
      raw.replace("Serial NO.:,C1-1", `Serial NO.:,${CODE}-S1-3`),
      "utf8",
    );
    const r3 = await post(
      pixel3,
      `${CODE}-S1-3_Cindy_perovskite_Light_Normal_Rev_1_143200.csv`,
      key,
    );
    ok("upload still accepted", r3.code === 200, JSON.stringify(r3.body));
    ok(
      "server says it kept the manual value",
      /kept the value/.test(r3.body.message),
      r3.body.message,
    );
    const after = await prisma.characterizationResult.findUnique({
      where: { id: result!.id },
    });
    ok(
      "manual 19.00 survived",
      (after?.metrics as Record<string, string>)?.["PCE (%)"] === "19.00",
      JSON.stringify(after?.metrics),
    );

    console.log(`\n── unknown serials land in a review queue, not the bin ──`);
    const stranger = await post(
      readFileSync(path.join(FIXTURES, "giantforce-manual-data.csv")),
      "C2-2_Cindy_Data.csv",
      key,
    );
    ok(
      "accepted but reported unmatched",
      stranger.code === 200 && stranger.body.status === "unmatched",
      JSON.stringify(stranger.body),
    );
    ok(
      "all four scans kept",
      stranger.body.scans === 4,
      String(stranger.body.scans),
    );
    ok(
      "message names the problem",
      /is not an experiment code/.test(stranger.body.message),
      stranger.body.message,
    );
    const orphans = await prisma.jvMeasurement.count({
      where: { instrumentId: rig.id, status: "UNMATCHED" },
    });
    ...
    const table = await post(
      readFileSync(path.join(FIXTURES, "giantforce-manual-table.csv")),
      "C2-2_Cindy_Table.csv",
      key,
    );
    ok(
      "422 so the agent stops retrying",
      table.code === 422,
      String(table.code),
    );
    ok(
      "explains why",
      /Save table/.test(table.body.message),
      table.body.message,
    );
    ...
    try {
      const ls = await post(
        readFileSync(path.join(FIXTURES, "lightsky-session.csv")),
        "123.xls",
        lsKey,
        {
      ...
      const lsRows = await prisma.jvMeasurement.findMany({
        where: { instrumentId: lsRig.id },
      });
      ok(
        "stored with module-scale metrics",
        (lsRows[0]?.metrics as { voc: number })?.voc > 11,
      );
      ok(
        "saved as .xls but parsed by content",
        lsRows.length === 3,
        String(lsRows.length),
      );
    } finally {
    ...
    try {
      const session1 = readFileSync(
        path.join(FIXTURES, "lightsky-session.csv"),
      );
      const first = await post(session1, "session1.csv", lsKey2, {
        sourceDir: "D:\\Lily\\2026",
      });
      ok(
        "first save: 3 new scans",
        first.body.scans === 3,
      ...
      const second = await post(session2, "session2.csv", lsKey2, {
        sourceDir: "D:\\Lily\\2026",
      });
      ok(
        "re-saved file is accepted, not 409",
        second.code === 200,
        String(second.code),
      );
      ok(
        "but adds 0 new scans",
        second.body.scans === 0,
        JSON.stringify(second.body),
      );
      ok(
        "and says so",
        /already recorded/.test(second.body.message),
        second.body.message,
      );
      const total = await prisma.jvMeasurement.count({
        where: { instrumentId: lsRig2.id },
      });
      ok("still exactly 3 measurements", total === 3, String(total));
      const files = await prisma.instrumentUpload.count({
        where: { instrumentId: lsRig2.id },
      });
    ...
    const early = Buffer.from(
      raw.replace("Serial NO.:,C1-1", `Serial NO.:,${LATE}-S1`),
      "utf8",
    );
    const earlyRes = await post(early, `${LATE}-S1_Cindy.csv`, key);
    ok(
      "arrives unmatched",
      earlyRes.body.status === "unmatched",
      JSON.stringify(earlyRes.body),
    );
    ok(
      "reason recorded",
      /No experiment 2026-001-9-98/.test(earlyRes.body.message),
      earlyRes.body.message,
    );
    ...
    const lateResult = await prisma.characterizationResult.findFirst({
      where: { sampleId: late.samples[0].id },
    });
    ok(
      "and the sample result is filled in",
      (lateResult?.metrics as Record<string, string>)?.["PCE (%)"] === "25.80",
    ...
    const shortFile = Buffer.from(
      raw.replace("Serial NO.:,C1-1", `Serial NO.:,${serial}-2`),
      "utf8",
    );
    const shortRes = await post(shortFile, `${serial}-2_Chloe.csv`, key);
    ok(
      "a scan typed with the short serial matches",
      shortRes.body.status === "stored",
      JSON.stringify(shortRes.body),
    );
    const m1 = await prisma.jvMeasurement.findFirstOrThrow({
      where: { serialKey: `${serial}-2` },
    });
    ...
    const stranded = await prisma.jvMeasurement.findUniqueOrThrow({
      where: { id: m1.id },
    });
    ok(
      "deleting samples strands the measurement (sampleId is nulled)",
      stranded.sampleId === null,
    );

    const healed = await rematchThroughApi(org.slug);
    ok(
      "the sweep re-attaches it",
      healed.matched === 1,
    ...
    ok(
      "back on S1 — the same sample by serial, a new row by id",
      after2.sample?.code === "S1" && after2.sampleId !== s1Before.id,
      JSON.stringify(after2),
    );
    const restored = await prisma.characterizationResult.findFirst({
      where: { sampleId: after2.sampleId! },
    });
    ok(
      "and the sample result is rebuilt",
      (restored?.metrics as Record<string, string>)?.["PCE (%)"] === "25.80",
    ...
    ok(
      "liveness recorded",
      !!beat?.lastSeenAt && beat.hostname === "DESKTOP-MGMS5H0",
      beat?.hostname,
    );
  } finally {
