...
type UserRow = { id: string; name: string; email: string; role: string; active: boolean; createdAt: string; materialAdmin: boolean; recipeAccess: boolean };
  ...
  return (
    <div className="space-y-6">
        ...
        <table className="w-full min-w-[640px] text-[12.5px]">
          <thead>
            <tr className="text-left text-[10px] uppercase text-muted border-b border-line">
              ...
              <th className="px-3.5 py-2 font-bold">{t("users.email")}</th>
              <th className="px-3.5 py-2 font-bold">{t("users.role")}</th>
              <th className="px-3.5 py-2 font-bold">{t("users.perms")}</th>
              <th className="px-3.5 py-2 font-bold">{t("users.joined")}</th>
            ...
            {users.map((u) => (
              <tr key={u.id} className={"border-b border-line last:border-0 " + (u.active ? "" : "opacity-45")}>
                <td className="px-3.5 py-2.5 font-medium">
                  {u.name}
                  {u.id === sessionUid && <span className="text-muted"> (you)</span>}
                </td>
                <td className="px-3.5 py-2.5 mono text-[11.5px]">{u.email}</td>
                ...
                <td className="px-3.5 py-2.5">
                  <div className="flex flex-col gap-1">
                    {([["materialAdmin", "users.permMaterials"], ["recipeAccess", "users.permRecipes"]] as const).map(([perm, label]) => (
                      <label key={perm} className="flex items-center gap-1.5 text-[11px] text-charcoal whitespace-nowrap">
                        <input
                          type="checkbox"
                          className="accent-[#95CA00]"
                          checked={u.role === "ADMIN" ? true : (u[perm] as boolean)}
                          disabled={busy || u.role === "ADMIN"}
                          onChange={async (e) => {
                            setBusy(true);
                            await setUserPermission(u.id, perm, e.target.checked);
                            setBusy(false);
                            router.refresh();
                          }}
                        />
                        {t(label as "users.permMaterials")}
                      </label>
                    ))}
                  </div>
                </td>
                <td className="px-3.5 py-2.5 mono text-muted">{u.createdAt}</td>
      ...
      <section className="bg-surface border border-line rounded-[6px] p-4">
        ...
        <p className="text-[11px] text-muted mb-2">{t("users.domainsHint")}</p>
        <div className="flex gap-2">
          <input className={inputCls + " max-w-md mono"} value={domains} onChange={(e) => setDomains(e.target.value)} />
          <button
            disabled={busy}
            onClick={async () => {
              setBusy(true);
          ...
          {savedFlash && <span className="text-[12px] text-brand-deep self-center">{t("profile.saved")}</span>}
        </div>
      </section>

      {/* Pending registrations (active OTP codes) */}
      <section className="bg-surface border border-line rounded-[6px] p-4">
        ...
        {pending.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {pending.map((p) => (
              <div key={p.email} className="flex items-center gap-3 border border-line rounded-[4px] px-3 py-2">
                <span className="mono text-[12px] flex-1">{p.email}</span>
...
// Admin creates a login directly — for colleagues who can't receive the
// passcode email yet. The password stays visible after creation so the
// admin can copy and hand it over.
