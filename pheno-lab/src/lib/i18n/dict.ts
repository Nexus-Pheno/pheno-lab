// English is the default language; Chinese translations are written for a
// materials-science lab audience (工艺/表征 terminology), not machine-translated.

export const en = {
  // App chrome
  "app.name": "Lab Data Platform",
  "nav.dashboard": "Dashboard",
  "nav.experiments": "Experiments",
  "nav.library": "Library",
  "nav.data": "Data",
  "nav.signout": "Sign out",
  "nav.back": "Back",
  "nav.profile": "Profile",

  // Login
  "login.email": "Email",
  "login.password": "Password",
  "login.signin": "Sign in",
  "login.signingin": "Signing in…",
  "login.invalid": "Invalid email or password.",

  // Roles / status
  "role.ADMIN": "Admin",
  "role.MANAGER": "Manager",
  "role.TECHNICIAN": "Technician",
  "status.DRAFT": "Draft",
  "status.IN_LAB": "In lab",
  "status.REVIEW": "In review",
  "status.COMPLETE": "Complete",
  "status.ARCHIVED": "Archived",

  // Experiment list
  "list.title": "Experiments",
  "list.experiments": "experiments",
  "list.experiment": "experiment",
  "list.assigned": "you are assigned to",
  "list.involved": "you created or are involved in",
  "list.newPlaceholder": "New experiment title…",
  "list.sortHint": "Click to sort, again to flip",
  "list.filterHint": "Filter…",
  "list.all": "All",
  "list.clearFilters": "Clear",
  "list.create": "Create",
  "list.code": "Code",
  "list.titleCol": "Title",
  "list.status": "Status",
  "list.createdBy": "Created by",
  "list.samples": "Samples",
  "list.steps": "Steps",
  "list.labels": "Labels",
  "list.updated": "Updated",
  "list.emptyStaff":
    "No experiments yet. Create one to design your first process flow.",
  "list.emptyTech":
    "No experiments assigned to you yet. Ask a manager to grant you access.",

  // Dashboard
  "dash.title": "Dashboard",
  "dash.titleTech": "My tasks",
  "dash.subtitle": "progress of all ongoing experiments",
  "dash.subtitleTech": "experiments assigned to you",
  "dash.kanban": "Kanban",
  "dash.cards": "Cards",
  "dash.list": "List",
  "dash.search": "Search experiments…",
  "dash.capture": "Capture data",
  "dash.duplicate": "Duplicate",
  "dash.duplicated": "Experiment duplicated.",
  "dash.new": "New experiment",
  "dash.campaign": "Campaign",
  "dash.project": "Team",
  "dash.allProjects": "All teams",
  "dash.noProject": "No team",
  "dash.needTitle":
    "Give the experiment a meaningful title before sending it to the lab — “Untitled experiment” cannot be found again or reported on.",
  "dash.needProject":
    "Pick a team (项目组) in settings before sending the experiment to the lab.",
  "dash.empty": "Nothing here yet.",
  "dash.open": "Open",
  "dash.dragHint": "Drag cards between columns to change status.",

  // Designer
  "designer.settings": "Settings",
  "designer.saveDraft": "Save as draft",
  "designer.sendToLab": "Send to lab",
  "designer.readonly": "Read-only",
  "designer.processLibrary": "Process library",
  "designer.railHintEdit":
    "Click a step to append it to the flow. Drag cards to reorder.",
  "designer.railHintView": "Process step types available in this lab.",
  "designer.characterization": "Characterization",
  "designer.processFlow": "Process flow",
  "designer.flowHint": "tested steps are highlighted",
  "designer.dragHint": "drag cards to reorder",
  "designer.charPlan": "Characterization plan",
  "designer.charHint": "runs after processing",
  "designer.emptySteps":
    "No process steps yet. Add the first step from the library on the left — start with how the substrate is cleaned.",
  "designer.emptyChars":
    "No characterization planned yet. Add measurements from the library on the left.",
  "designer.saved": "All changes saved",
  "designer.saving": "Saving…",
  "designer.saveFailed": "Save failed — retry your last edit",
  "designer.samples": "samples",
  "designer.processSteps": "process steps",
  "designer.characterizations": "characterizations",
  "designer.selectHint": "Select a card in the flow to inspect and edit it.",
  "designer.selectHintEdit":
    "Use the test plan to define what this experiment varies — tested steps light up green.",

  // Science strip
  "sci.observation": "Observation",
  "sci.problem": "Problem",
  "sci.hypothesis": "Hypothesis",
  "sci.conclusion": "Conclusion",
  "sci.observationPh": "What did you observe that motivated this experiment?",
  "sci.problemPh": "What question is this experiment answering?",
  "sci.hypothesisPh": "What do you expect to happen, and why?",
  "sci.conclusionLocked": "Unlocks when the experiment is marked complete.",
  "sci.aiSummary": "AI summary",
  "sci.aiHint":
    "Let AI analyze the full experiment record before you write the conclusion.",
  "sci.aiGenerate": "Generate",
  "sci.aiRegenerate": "Regenerate",
  "sci.aiBusy": "Analyzing…",
  "sci.aiBackground":
    "Runs in the background — you can leave this page and come back.",
  "sci.aiLocked": "Unlocks with the conclusion once the experiment completes.",
  "dash.viewMore": "View more ({n})",
  "activity.title": "Activity",
  "activity.online": "Online now",
  "activity.nobody": "Nobody online right now.",
  "activity.empty": "No recorded activity yet.",
  "activity.collapse": "Collapse",
  "act.created": "created",
  "act.updated": "updated",
  "act.deleted": "removed",
  "act.published": "published",
  "act.imported": "imported",
  "act.duplicated": "duplicated",
  "act.aiSummary": "generated AI summary for",
  "act.changed": "changed",
  "sci.aiFailed":
    "No AI model configured or the call failed — try again or check the AI provider in Profile.",
  "sci.aiDisclaimer": "AI-generated; verify numbers before citing",

  // Test plan
  "plan.title": "Test plan",
  "plan.question":
    "which variables are you testing, in which processes, with which groups and control?",
  "plan.testing": "Testing",
  "plan.in": "in",
  "plan.none": "No variables defined yet — what are you testing?",
  "plan.edit": "Edit test plan",
  "plan.define": "Define test plan",
  "plan.variableType": "type",
  "plan.variable": "Variable",
  "plan.parameterKind": "Process parameter",
  "plan.materialKind": "Material",
  "plan.process": "Process",
  "plan.parameterTested": "Parameter being tested",
  "plan.selectParameter": "Select parameter…",
  "plan.customParameter": "Custom parameter…",
  "plan.materialSlot": "Material slot name",
  "plan.unit": "Unit",
  "plan.addVariable": "Add variable",
  "plan.group": "Group",
  "plan.extras": "Extras",
  "plan.errors": "Trash / problem",
  "plan.dropHere": "drag substrates here",
  "plan.substrates": "Substrate batch",
  "plan.substrateCount": "Substrates to prepare",
  "plan.substrateMaterial": "Substrate material",
  "plan.substrateHint":
    "Drag each substrate into its group. Spares wait in Extras; scrapped ones go to Error — swap in a spare without renumbering.",
  "plan.regroup": "Regroup substrates",
  "plan.samples": "Samples",
  "plan.control": "Control",
  "plan.controlWord": "control",
  "plan.selectMaterial": "Select material…",
  "plan.addMaterial": "＋ Add new material…",
  "plan.applyAll": "Apply to all groups",
  "plan.addGroup": "Add group",
  "plan.total": "Total samples:",
  "plan.autoCalc": "(auto-calculated)",
  "plan.applyNote":
    "Applying regenerates samples and highlights each tested variable on its process step (added if missing). Variables removed from the plan are cleaned off the flow.",
  "plan.cancel": "Cancel",
  "plan.apply": "Apply test plan",

  // Inspector
  "insp.step": "Step",
  "insp.char": "Characterization",
  "insp.preset": "Preset — commonly used",
  "insp.applyPreset": "Apply a preset…",
  "insp.used": "used",
  "insp.presetApplied": "Preset applied — save changes to keep it.",
  "insp.stepName": "Step name",
  "insp.name": "Name",
  "insp.equipment": "Equipment",
  "insp.instrument": "Instrument",
  "insp.noEquipment": "No equipment",
  "insp.noInstrument": "No instrument",
  "insp.equipmentHintEmpty":
    "No {process} equipment yet — add it in the Library under this process.",
  "insp.equipmentHint":
    "Only equipment under this process is selectable. Selecting a machine loads its parameter set.",
  "insp.parameters": "Process parameters",
  "insp.parameter": "Parameter",
  "insp.value": "Value",
  "insp.unit": "unit",
  "insp.addParameter": "Add parameter",
  "insp.vary": "Vary across groups",
  "insp.varied": "Tested variable — varies by group",
  "insp.removeVariation": "Remove variation",
  "insp.environment": "Environment",
  "insp.noEnvironment": "No environment specified",
  "insp.materials": "Materials deposited",
  "insp.noMaterial": "No material deposited in this step.",
  "insp.selectMaterial": "Select material…",
  "insp.addMaterial": "Add material",
  "insp.addMaterialOpt": "＋ Add material…",
  "insp.amountPh": "amount",
  "insp.amountMixPh": "e.g. 60 mol%",
  "insp.mixHint":
    "Multiple materials in one process — give the share of each (e.g. mol%).",
  "insp.materialFilterHint":
    "Only materials categorized under {process} are shown.",
  "insp.settings": "Settings",
  "insp.setting": "Setting",
  "insp.addSetting": "Add setting",
  "insp.sampleScope": "Sample scope",
  "insp.allSamples": "All samples",
  "insp.perGroup": "One sample per variation group",
  "insp.notes": "Notes",
  "insp.save": "Save changes",
  "insp.cancel": "Cancel changes",
  "insp.saveAsPreset": "Save as preset",
  "insp.presetNamePrompt": "Preset name:",
  "insp.newMaterialPrompt": "New material name:",

  // Cards
  "card.noEquipment": "No equipment assigned",
  "card.noInstrument": "No instrument assigned",
  "card.testing": "Testing:",
  "card.allSamples": "All samples",
  "card.perGroup": "1 per group",
  "card.delete": "Delete",
  "card.deleteQ": "Delete?",

  // Settings modal
  "set.title": "Experiment settings",
  "set.expTitle": "Title",
  "set.status": "Status",
  "set.statusHint":
    "Draft → In lab when the plan is ready for technicians. The conclusion unlocks at Complete.",
  "set.participants": "Participants — who is involved",
  "set.creator": "Creator",
  "set.remove": "Remove",
  "set.grant": "＋ Grant access to…",
  "set.autoLabels": "Auto-generated labels",
  "set.autoLabelsHint":
    "Generated from the processes, equipment, materials, and variables this experiment uses.",
  "set.project": "Team (项目组)",
  "set.projectNone": "— Not assigned —",
  "set.projectNew": "New team",
  "set.projectAdd": "Add",
  "set.projectHint":
    "Which team this experiment belongs to — filled in from your own team by default. Required before it goes to the lab; it groups and filters the board, and never limits which experiments analysis may compare.",
  "set.campaign": "Campaign / group",
  "set.campaignHint":
    'Optional — group related experiments under one larger campaign, e.g. "Anneal optimization Q3". Searchable from the board.',
  "nav.analysis": "Analysis",
  "an.askTab": "Ask",
  "an.manualTab": "Compare one condition",
  "an.askSubtitle":
    "Ask a question. The app finds the experiments it concerns, computes the comparisons, and the model writes the reading — factors together, not one at a time.",
  "an.askTitle": "Ask the lab's history",
  "an.askHint":
    "Any question about what the lab has tried: a material, a process, a combination, a trend. Retrieval is across every experiment in the lab; nothing is pre-selected. Answers take one to three minutes and stay here for everyone.",
  "an.askPlaceholder":
    "e.g. Does blade coating beat spin coating for the HTL, and does it depend on the SAM used?",
  "an.askShortcut": "⌘/Ctrl + Enter to ask",
  "an.askButton": "Ask",
  "an.noHistory": "No questions asked yet.",
  "an.running": "reading the data…",
  "an.runFailed": "could not complete",
  "an.basedOn": "{experiments} experiments · {samples} samples",
  "an.experimentsRead": "Experiments read",
  "an.terms": "Searched for",
  "an.recipeSource": "from recipes",
  "an.title": "Cross-experiment analysis",
  "an.subtitle":
    "Every experiment in the lab, pooled by the conditions they varied. Pick a condition to see how its values compare across batches.",
  "an.condition": "Condition",
  "an.pickCondition": "— Pick a condition —",
  "an.metric": "Metric",
  "an.from": "From",
  "an.to": "To",
  "an.filter": "Filter",
  "an.filterHint": "Material, process, code or title…",
  "an.reset": "Clear",
  "an.scopeLine": "{experiments} experiments · {samples} measured samples",
  "an.truncated":
    "Showing the most recent 400 experiments. Narrow the date range for a complete answer.",
  "an.emptyScope": "No measured samples in this scope yet.",
  "an.emptyCondition":
    "Pick a condition above. The list holds every parameter the lab varied and measured more than one value for.",
  "an.pooled": "Pooled comparison",
  "an.pooledHint":
    "Every measured sample that ran under each value, across all the experiments in scope.",
  "an.value": "Value",
  "an.samples": "Samples",
  "an.experiments": "Experiments",
  "an.mean": "Mean",
  "an.median": "Median",
  "an.best": "Best",
  "an.sd": "SD",
  "an.perBatch": "Batch by batch",
  "an.perBatchHint":
    "The same comparison inside each experiment, where the rest of the process was held constant. A conclusion that repeats here is worth far more than the pooled mean.",
  "an.winner": "Best in batch",
  "an.wins": "Wins",
  "an.winsHint": "How often each value came out on top within its own batch.",
  "an.plots": "Distribution",
  "an.plotsHint":
    "Box = quartiles, whiskers = range, triangle = mean, dots = individual samples.",
  "an.anomalies": "Flagged",
  "an.export": "Download table (CSV)",
  "an.caution":
    "Pooling across batches mixes everything else that differed between them. Treat the batch-by-batch table as the stronger evidence.",
  "an.ai": "AI reading",
  "an.aiHint":
    "DeepSeek reads the computed table above — not the raw curves — and writes up what repeats, what conflicts, and what is worth running next.",
  "an.ask": "Analyse this scope",
  "an.asking": "Reading the data…",
  "an.aiEmpty": "Pick a condition first, so there is something to read.",
  "an.aiFailed": "The analysis could not be completed. Try again.",
  "an.aiStale": "Scope changed — rerun for an answer that matches.",
  "an.generated": "Generated {when} · {model}",
  "anom.champion": "Champion device: {sample} ({group}) at PCE {value}.",
  "anom.negative":
    "{sample}: {metric} is negative ({value} Ω) — a measurement artifact, not a device property. Its PCE/Voc/Jsc/FF are still valid.",
  "anom.outlier":
    "{sample} (group {group}): PCE {value} sits outside its group's range (median {median}).",
  "proj.title": "Teams (项目组)",
  "proj.hint":
    "The organization's teams, as on the org chart. Experiments are filed under one; each person's team is set on the organization page and becomes the default for their new experiments. Retire a disbanded team to keep it out of the picker without touching its experiments.",
  "proj.name": "Team name",
  "proj.add": "Add team",
  "proj.experiments": "experiments",
  "proj.active": "Active",
  "proj.inactive": "Inactive",
  "proj.activate": "Reactivate",
  "proj.deactivate": "Deactivate",
  "proj.rename": "Rename",
  "proj.save": "Save",
  "proj.cancel": "Cancel",
  "proj.empty": "No teams yet.",
  "set.delete": "Delete experiment",
  "set.deleteQ": "Delete this experiment and all its data?",
  "set.deleteYes": "Delete",
  "set.deleteNo": "Cancel",

  // Library
  "lib.processes": "Processes",
  "lib.processesHint":
    "the overarching layer — equipment and materials live under each process",
  "lib.characterization": "Characterization",
  "lib.charHint": "measurement processes and their instruments",
  "lib.equipmentFor": "Equipment for",
  "lib.materialsFor": "Materials for",
  "lib.paramsFor": "Processing parameters for",
  "lib.paramsHint":
    "The parameters recorded for every step of this process. Each is tagged as a processing parameter in the database. New steps start with these; machine parameters overlay them when equipment is selected.",
  "lib.noEquipment": "No equipment under this process yet.",
  "lib.noMaterials": "No materials categorized under this process yet.",
  "lib.addEquipment": "Add equipment",
  "lib.addMaterial": "Add material",
  "lib.addProcess": "Add process",
  "lib.renameProcess": "Rename process",
  "lib.archiveProcess": "Archive process",
  "lib.restoreProcess": "Restore process",
  "lib.nickname": "Nickname (what colleagues call it)",
  "lib.nicknamePh": "e.g. 小蒸发台 / the blue evaporator",
  "lib.equipmentName": "Equipment name",
  "lib.manufacturer": "Manufacturer",
  "lib.modelNumber": "Model number",
  "lib.assetTag": "Asset tag",
  "lib.location": "Location",
  "lib.noLocation": "No location",
  "lib.addLocation": "＋ Add location…",
  "lib.photo": "Photo",
  "lib.uploadPhoto": "Upload photo",
  "lib.replacePhoto": "Replace photo",
  "lib.removePhoto": "Remove",
  "lib.uploading": "Uploading…",
  "lib.envDetails": "Details",
  "lib.envDetailsHint":
    "The enclosure itself — make, model, chamber layout. Conditions above are what an operator records each run.",
  "lib.specSheets": "Spec sheets",
  "lib.specSheetsHint":
    "The manufacturer's original documents for this machine — manuals, datasheets, acceptance reports.",
  "lib.addSpecSheet": "Add document",
  "lib.noSpecSheets": "No documents attached",
  "lib.machineParams": "Specifications",
  "lib.machineParamsHint":
    "Reference values from the spec sheet — shown on the equipment card only, never loaded into steps.",
  "lib.workParams": "Process parameters (used in experiments)",
  "lib.workParamsHint":
    "The knobs techs actually set on this machine day to day. When a step or test-plan variable selects this machine, these definitions are what gets offered and pre-filled.",
  "lib.materialName": "Material name",
  "lib.composition": "Composition / recipe",
  "lib.supplier": "Supplier",
  "lib.lotNumber": "Lot number",
  "lib.edit": "Edit",
  "lib.close": "Close",
  "lib.archive": "Archive",
  "lib.archived": "Archived",
  "lib.unarchive": "Restore",
  "lib.restore": "Restore",
  "lib.cancel": "Cancel",
  "lib.saveChanges": "Save changes",
  "lib.saveParams": "Save parameters",
  "lib.environments": "Environments",
  "lib.environmentsHint":
    "your lab's environments and the conditions mapped in each",
  "lib.envName": "Environment name",
  "lib.conditions": "Tracked conditions",
  "lib.conditionsHint":
    "The conditions mapped in this environment — e.g. O₂/H₂O ppm and temperature for a glovebox, or particle count and humidity for a clean room.",
  "lib.addEnvironment": "Add environment",
  "lib.locations": "Locations",
  "lib.locationsHint":
    "organization locations rarely change — set them once, select everywhere",
  "lib.addLocationBtn": "Add location",
  "lib.newLocationPh": "New location, e.g. Building A — Perovskite lab",
  "lib.noLocations": "No locations yet.",
  "lib.presets": "Presets",
  "lib.presetsHint":
    'click a preset to edit it — saved from the designer via "Save as preset"',
  "lib.presetName": "Preset name",
  "lib.paramValues": "Parameter values",
  "lib.presetSettings": "Settings",
  "lib.noPresets":
    'No presets yet — configure a step in the designer and use "Save as preset".',
  "lib.stepKind": "Process step",
  "lib.charKind": "Characterization",
  "lib.process": "Process",
  "lib.defName": "name",
  "lib.defUnit": "Unit",
  "lib.defDefault": "Default value",
  "lib.add": "Add",
  "lib.equipmentCount": "equipment",
  "lib.materialsCount": "materials",
  "lib.processName": "Process name",
  "lib.icon": "Icon",

  // Data table
  "data.title": "Data table",
  "data.subtitle":
    "One row per sample — parameters resolved per variation group, tagged and export-ready.",
  "data.filter": "Filter rows…",
  "data.search":
    "Search a person, material, process, sample, formula… or ask a question",
  "data.showMatches": "Show matches",
  "data.hideMatches": "Hide matches",
  "data.searchGo": "Search",
  "data.pageOf": "Page {page} of {pages}",
  "data.expMatching": "{n} experiments",
  "data.cells": "cells on this page",
  "data.rows": "rows",
  "data.columns": "columns",
  "data.export": "Export CSV",
  "data.noRows": "No rows match.",

  // Profile
  "profile.title": "Profile",
  "profile.displayName": "Display name",
  "profile.handle": "Handle",
  "profile.handlePh": "e.g. @mike",
  "profile.save": "Save profile",
  "profile.loginInfo": "Login information",
  "profile.email": "Email",
  "profile.password": "Password",
  "profile.currentPassword": "Current password",
  "profile.newPassword": "New password",
  "profile.confirmPassword": "Confirm new password",
  "profile.changePassword": "Change password",
  "profile.passwordChanged": "Password changed.",
  "profile.passwordMismatch": "New passwords do not match.",
  "profile.passwordWrong": "Current password is incorrect.",
  "profile.organization": "Organization",
  "profile.role": "Role",
  "profile.memberSince": "Member since",
  "profile.language": "Language",
  "profile.langHint": "Applies to the whole app for your account.",
  "profile.stats": "Your stats",
  "profile.statExperiments": "Experiments involved",
  "profile.statCompleted": "Experiments completed",
  "profile.statSamples": "Samples planned",
  "profile.statDataPoints": "Data points saved",
  "profile.statPresets": "Presets created",
  "profile.statDisk": "Server disk used · {used} of {total} GB",
  "tr.showOriginal": "original",
  "tr.showTranslation": "translation",
  "tr.translate": "translate",
  "tr.translating": "translating…",
  "profile.saved": "Saved.",

  // Feedback
  "fb.title": "Report a bug / submit feedback",
  "fb.kind": "Type",
  "fb.bug": "Bug report",
  "fb.feedback": "Feedback",
  "fb.message": "What happened, or what could be better?",
  "fb.messagePh":
    "Describe the issue or idea. For bugs: what did you expect, what happened instead?",
  "fb.screenshot": "Screenshot (optional)",
  "fb.attach": "Attach screenshot",
  "fb.attached": "Screenshot attached",
  "fb.includeErrors": "Include recent error logs automatically",
  "fb.submit": "Submit",
  "fb.submitted": "Thanks — your report has been recorded.",
  "fb.adminTitle": "Feedback & bug reports",
  "fb.adminHint":
    "collected across the organization — exportable for agents to fix",
  "fb.exportJson": "Export JSON",
  "fb.open": "Open",
  "fb.resolved": "Resolved",
  "fb.markResolved": "Mark resolved",
  "fb.reopen": "Reopen",
  "fb.none": "No feedback yet.",
  "nav.feedback": "Feedback",
  "fb.boardTitle": "Feedback",
  "fb.boardHint":
    "One problem per item — describe it, attach screenshots, and track the admin's verdict here.",
  "fb.newItem": "Report a problem or idea",
  "fb.titlePh":
    "Short title, e.g. “Material dropdown is clipped in the test plan”",
  "fb.addShot": "Screenshot",
  "fb.st.open": "Open",
  "fb.st.approved": "Approved",
  "fb.st.rejected": "Rejected",
  "fb.st.implemented": "Implemented",
  "fb.st.resolved": "Implemented",
  "fb.st.verified": "Verified",
  "fb.st.reopened": "Reopened",
  "fb.tab.open": "Open",
  "fb.tab.approved": "Approved",
  "fb.tab.rejected": "Rejected",
  "fb.tab.implemented": "Implemented",
  "fb.tab.all": "All",
  "fb.tab.verified": "Verified",
  "fb.verifyBanner":
    "{n} implemented item(s) are waiting for your test & confirmation — tap to review the patch notes.",
  "fb.implNote": "Patch notes",
  "fb.implNotePh":
    "What changed and how to use it — the reporter sees this and tests against it.",
  "fb.implNoteRequired": "Write patch notes first — the reporter needs them.",
  "fb.disputeNote": "Reporter sent it back",
  "fb.verifyAsk":
    "This was implemented — please test it. Happy with the change?",
  "fb.autoOn": "No response = auto-approved on",
  "fb.verifyOk": "✓ Works, approve",
  "fb.verifyNo": "Still not right",
  "fb.verifyNoPh": "What is still wrong or missing?",
  "fb.verifySend": "Send back",
  "fb.verifiedAuto": "Auto-approved after 7 days",
  "fb.verifiedYou": "You approved this change",
  "fb.reopenedWait": "Sent back — waiting for another pass.",
  "fb.adminNote": "Admin comment",
  "fb.adminNotePh": "Scoping notes, priorities, or why this was rejected…",
  "fb.approve": "Approve",
  "fb.reject": "Reject",
  "fb.markImplemented": "Mark implemented",
  "fb.saveNote": "Save comment",
  "fb.viewAll": "View all feedback (admin)",
  "fb.errorLog": "Error log",
  "fb.page": "Page",

  // Capture portal
  "cap.title": "Data capture",
  "cap.sample": "Sample",
  "cap.progress": "Progress",
  "cap.planned": "Planned",
  "cap.actual": "Actual",
  "cap.confirm": "Confirm step",
  "cap.confirmed": "Captured",
  "cap.edit": "Edit capture",
  "cap.note": "Note",
  "cap.notePh": "Anything unusual? Deviations, observations…",
  "cap.flag": "Flag issue",
  "cap.flagged": "Flagged",
  "cap.photo": "Photo",
  "cap.photoAttached": "Photo attached",
  "cap.environment": "Environment conditions",
  "cap.steps": "Process steps",
  "cap.results": "Characterization results",
  "cap.metric": "Metric",
  "cap.value": "Value",
  "cap.addMetric": "Add metric",
  "cap.saveResult": "Save result",
  "cap.resultSaved": "Result saved",
  "cap.notInLab":
    'This experiment is not in the lab yet. Set its status to "In lab" to start capturing data.',
  "cap.open": "Open capture portal",
  "cap.allDone": "All steps captured for this sample.",
  "cap.doneOf": "captured",
  "cap.stepOf": "Step",
  "cap.of": "of",
  "cap.prev": "Previous",
  "cap.next": "Next",
  "cap.swipeHint": "Swipe between steps · tap a dot to jump",
  "cap.run": "Run",
  "cap.newRun": "Start new run",
  "cap.newRunHint":
    "Execute the same plan again with a fresh set of actuals and results (e.g. a reproducibility batch).",
  "cap.deleteRun": "Delete current run",
  "cap.deleteRunQ": "Delete Run {run}?",
  "cap.confirmDeleteRun": "Confirm delete run",
  "cap.keepOneRun": "At least one run must remain",

  // Mobile input portal
  "portal.title": "Lab input portal",
  "portal.subtitle": "experiments ready for data capture",
  "portal.captureNow": "Tap to capture",
  "portal.progress": "captured",
  "portal.empty":
    'No experiments are in the lab right now. When one is set to "In lab", it appears here.',
  "portal.toDesktop": "Switch to full dashboard",
  "portal.install": "Install this app",
  "portal.iosTitle": "Add Pheno Lab to your home screen",
  "portal.iosStep1": "Tap the Share button in the browser toolbar",
  "portal.iosStep2": 'Choose "Add to Home Screen"',
  "portal.iosHint":
    "The icon opens full-screen like an app, with the Pheno logo.",
  "portal.toPortal": "Lab portal",
  "portal.other": "Other experiments",
  "cap.applyTo": "Apply to",
  "cap.batchAll": "All samples",
  "cap.batchGroup": "Group",
  "cap.batchOne": "Single sample",
  "cap.batchHint":
    "Tap a group or a single sample to choose what you're recording — one confirm covers the selection. Steps with tested variables are captured per group.",
  "cap.pickScope": "Select a group or a sample above to start recording.",
  "cap.complete": "Complete experiment",
  "cap.completeHint": "All data captured — mark this experiment as complete.",
  "cap.completeMissing":
    "captures still missing — you can still complete now and add them later.",
  "cap.completeQ": "Mark as complete?",
  "cap.completed": "Experiment completed",
  "cap.viewReport": "View report",
  "designer.addStep": "Add step",
  "designer.addChar": "Add characterization",
  "cap.clear": "Clear capture",
  "cap.clearQ": "Clear capture?",
  "users.add": "Add member",
  "users.addHint":
    "Create a login directly — useful while passcode emails are not yet configured. Share the credentials with your colleague; they can change the password in their profile.",
  "users.password": "Password",
  "users.generate": "Generate password",
  "users.create": "Create account",
  "users.created": "Account created — share these credentials:",
  "users.emailTaken": "An account with this email already exists.",
  "users.badInput":
    "Check the email address and use a password of at least 8 characters.",
  "users.pwShort": "Password must be at least 8 characters.",
  "mat.title": "Materials",
  "mat.subtitle":
    "the lab's material wiki — maintained by material administrators",
  "mat.search": "Search materials (name, formula, CAS)...",
  "mat.readonlyHint":
    "Anyone can add materials. Edits to existing materials require steward approval.",
  "mat.add": "Add",
  "mat.new": "New material",
  "mat.name": "Name",
  "mat.category": "Category",
  "mat.formula": "Molecular formula / structure",
  "mat.smiles": "SMILES (organic molecules)",
  "mat.smilesHint":
    "Structure notation — the 2D structure is drawn from this. Leave empty for salts and metals.",
  "mat.structure": "Molecular structure",
  "mat.mw": "MW (g/mol)",
  "mat.purity": "Purity",
  "mat.vendor": "Vendor",
  "mat.lot": "Batch",
  "mat.props": "Characteristics",
  "mat.propName": "Property (e.g. HOMO (eV))",
  "mat.addProp": "Add characteristic",
  "mat.notes": "Notes",
  "mat.emptyCat": "No materials in this category yet.",
  "mat.noMatch": "No matching material.",
  "mat.categories": "Categories",
  "mat.catNew": "New category",
  "mat.catNamePh": "Category name",
  "mat.catDeleteQ": "Delete this category? Its materials move to:",
  "mat.catMoveTo": "Select a category…",
  "mat.catDelete": "Delete category",
  "mat.cat.SAM": "Wet-process SAM materials",
  "mat.cat.PRECURSOR": "Wet-process perovskite precursors",
  "mat.cat.EVAPORATION": "Thermal evaporation materials",
  "mat.cat.SPUTTER": "Sputter materials",
  "mat.cat.ALD": "ALD materials",
  "mat.cat.SOLVENT": "Solvents",
  "mat.cat.OTHER": "Other / prepared solutions",
  "rec.title": "Recipes",
  "rec.subtitle": "proprietary recipes — viewing and stewardship are separate",
  "rec.add": "Add recipe",
  "rec.new": "New recipe",
  "rec.name": "Recipe name",
  "rec.summary": "Recipe summary",
  "rec.summaryHint":
    'Visible with recipe contents, e.g. "1.68 eV wide-gap triple-cation"',
  "rec.components": "Components",
  "rec.component": "Component",
  "rec.amount": "Amount",
  "rec.addComponent": "Add component",
  "rec.solvents": "Solvent system",
  "rec.concentration": "Concentration",
  "rec.procedure": "Preparation procedure",
  "rec.lockedHint":
    "Everyone can create and select recipes. Creators and stewards can read their contents; recipe access unlocks approved recipes. Only stewards edit and approve.",
  "rec.hidden": "Contents hidden — recipe access required",
  "rec.empty": "No recipes yet.",
  "users.perms": "Extra permissions",
  "users.permMaterials": "Materials admin",
  "users.permRecipes": "Recipe access",
  "insp.searchMaterial": "Type to search materials...",
  "insp.createMaterial": "Create material",
  "insp.materialSearchHint":
    "Search finds close matches even if the spelling is slightly off. New materials are added by material administrators in the Library.",
  "insp.recipe": "Recipe",
  "insp.noRecipe": "No recipe",
  "insp.recipeHidden":
    "Recipe selected — contents visible to recipe-access members only.",
  "wf.assignee": "Assigned technician",
  "wf.assigneeHint":
    "Whoever is designated here runs the experiment in the lab and can capture its data. Required before sending it to the lab.",
  "wf.unassigned": "Not assigned yet",
  "wf.startLab": "Send to lab",
  "wf.needAssignee":
    "Assign someone before sending this experiment to the lab.",
  "wf.submit": "Submit for review",
  "wf.submitTitle": "Hand back to your manager",
  "wf.submitHint":
    "The manager reviews the captured data and closes the experiment.",
  "wf.submitNote": "Notes for the manager (optional)",
  "wf.submitted": "Submitted for review",
  "wf.submittedBy": "Submitted",
  "wf.awaiting": "Awaiting your approval",
  "wf.reviewTitle": "Manager review",
  "wf.reviewHint":
    "Approving seals the evidence pack — plan, captured data, photos and results — and closes the experiment.",
  "wf.reviewNote": "Closing notes / thoughts on this experiment",
  "wf.approve": "Approve & close",
  "wf.approveQ": "Approve and close?",
  "wf.requestChanges": "Send back to lab",
  "wf.changesQ": "Send back with these notes?",
  "wf.approvedBy": "Approved by",
  "wf.evidence": "Evidence pack",
  "wf.evidenceHint": "sealed on approval",
  "dash.newAs": "Create as",
  "dash.newReal": "Real experiment",
  "dash.newTest": "Test",
  "dash.fromTpl": "From template / previous",
  "dash.tplSection": "Pinned templates",
  "dash.recentSection": "My recent experiments",
  "dash.tplEmpty": "No templates or recent experiments to start from yet.",
  "dash.tplChip": "Template",
  "dash.pinTpl": "Pin as template",
  "dash.unpinTpl": "Unpin template",
  "trash.title": "Recycle bin",
  "trash.subtitle":
    "Deleted experiments wait here about 30 days before being removed for good. Restoring brings everything back — samples, data, comments.",
  "trash.empty": "The recycle bin is empty.",
  "trash.restore": "Restore",
  "trash.purge": "Delete forever",
  "trash.purgeQ": "Permanently delete?",
  "trash.deletedBy": "Deleted by",
  "trash.purgeOn": "auto-purge",
  "test.title": "Test data",
  "test.subtitle":
    "experiments created in test mode — hidden from every real view",
  "test.explain":
    "Test experiments never appear on the board, in the data table, the lab portal or profile statistics. They exist so you can try the app with throwaway work, and they can all be removed in one action.",
  "test.badge": "Test",
  "test.experiments": "test experiment(s)",
  "test.empty": "No test experiments.",
  "test.clear": "Clear all test data",
  "test.clearNow": "Delete",
  "test.confirm":
    "Permanently delete {n} test experiment(s) and all their samples, runs and results? Type DELETE to confirm.",
  "test.cleared": "Deleted {n} test experiment(s).",
  "test.promote": "Mark as real",
  "test.promoteHint": "Move this experiment into the real database",
  "test.nav": "Test data",
  "sum.dataPoints": "Data points",
  "sum.dataPointsHint":
    "实验室存储的每条测量曲线上的每一个 (x, y) 数据点——累计总数。",
  "sum.experiments": "Experiments",
  "sum.experimentsHint": "Every sample counts as one experiment.",
  "sum.samples": "Samples",
  "sum.results": "Measurements",
  "sum.runs": "Runs",
  "sum.materials": "Materials",
  "sum.recipes": "Formulas",
  "sum.files": "Files",
  "sum.search": "Search",
  "sum.searching": "Searching…",
  "sum.clear": "Clear",
  "sum.searchPh":
    "Search a material, process, formula, sample or ask a question…",
  "sum.found": "{n} related experiment(s)",
  "sum.none": "Nothing matched.",
  "ai.title": "AI models",
  "ai.subtitle":
    "keys are stored on the server and never shown again — only the last few characters",
  "ai.add": "Add model",
  "ai.label": "Name",
  "ai.provider": "Provider",
  "ai.baseUrl": "Base URL",
  "ai.model": "Model",
  "ai.key": "API key",
  "ai.keyKeep": "Leave blank to keep the stored key",
  "ai.active": "Active",
  "ai.use": "Use this",
  "ai.test": "Test",
  "ai.testing": "Testing…",
  "ai.save": "Save",
  "ai.cancel": "Cancel",
  "ai.remove": "Remove",
  "ai.removeQ": "Remove this model?",
  "ai.empty":
    "No model configured. Natural-language search falls back to keyword search until one is added.",
  "ai.never": "never tested",
  "ai.nav": "AI models",
  "ai.loadModels": "Load models",
  "ai.loading": "Loading…",
  "ai.liveModels": "{n} models offered by this provider right now",
  "ai.modelHint":
    "Paste a key and press Load models to see what this provider currently offers.",
  "exp.title": "Data exports",
  "exp.subtitleAdmin": "approve requests and review who has taken data out",
  "exp.subtitleMine": "your export requests and their status",
  "exp.request": "Request export",
  "exp.send": "Send request",
  "exp.reasonPh": "Why do you need this export?",
  "exp.requested": "Export requested — an administrator will review it.",
  "exp.alreadyPending": "You already have a request waiting for approval.",
  "exp.denied": "That export request was denied.",
  "exp.logged": "Export downloaded and recorded in the log.",
  "exp.pending": "Awaiting approval",
  "exp.noPending": "No requests waiting.",
  "exp.history": "Export log",
  "exp.noHistory": "No exports yet.",
  "exp.approve": "Approve",
  "exp.deny": "Deny",
  "exp.notePh": "Note (optional)",
  "exp.decidedBy": "decided by",
  "exp.downloaded": "downloaded",
  "exp.status.PENDING": "Pending",
  "exp.status.APPROVED": "Approved",
  "exp.status.DENIED": "Denied",
  "exp.nav": "Data exports",
  "ing.title": "Ingestion",
  "ing.subtitle":
    "review and publish facts extracted from documents and old data",
  "ing.howTitle": "How this works",
  "ing.howBody":
    "Hand your files (formula sheets, material datasheets, equipment manuals, old experiment records) to the agent. It extracts the facts and stages them here — nothing reaches the live library automatically. Review each item, correct anything wrong, then publish or reject it.",
  "ing.pending": "Awaiting review",
  "ing.reviewed": "Reviewed",
  "ing.emptyPending": "Nothing waiting for review.",
  "ing.emptyDone": "Nothing reviewed yet.",
  "ing.review": "Review",
  "ing.publish": "Approve & publish",
  "ing.reject": "Reject",
  "ing.rejectQ": "Reject this item?",
  "ing.saveDraft": "Save edits",
  "ing.reviewNote": "Review note",
  "ing.agentNote": "Agent note",
  "ing.extras": "Other extracted fields (not published)",
  "ing.published": "Published",
  "ing.rejected": "Rejected",
  "ing.by": "by",
  "ing.process": "Process",
  "ing.pickProcess": "Select a process…",
  "ing.kind.MATERIAL": "Material",
  "ing.kind.EQUIPMENT": "Equipment",
  "ing.kind.EXPERIMENT": "Experiment",
  "ing.expTitle": "Experiment title",
  "ing.operator": "Operator",
  "ing.scale": "Scale",
  "ing.batch": "Batch",
  "ing.date": "Date",
  "ing.steps": "Process steps",
  "ing.characterizations": "Characterisations",
  "ing.samples": "Samples",
  "ing.withMetrics": "with metrics",
  "ing.rawFiles": "raw files",
  "ing.andMore": "…and {n} more samples",
  "ing.kind.EXPERIMENT_": "",
  "ing.kind.FORMULA": "Formula",
  "ing.kind.ENVIRONMENT": "Environment",
  "ing.kind.PRESET": "Process recipe",
  "ing.presetName": "Recipe name",
  "ing.manage": "Ingestion (staged data)",
  "ing.composition": "Composition",
  "ing.bandGap": "Band gap (eV)",
  "ing.role": "Role",
  "ing.formulaNotes": "Notes from the source",
  "ing.inLibrary": "in library",
  "ing.matches": "in library as",
  "ing.notInLibrary": "not in library",
  "ing.allKnown": "All components already exist in the materials library.",
  "ing.unknownCount":
    "components are not in the materials library — publishing this formula will not create them.",
  "ing.dupFormula":
    "A recipe with this name already exists — publishing will update it in place.",
  "ing.dupTitle": "Already in your library",
  "ing.dupBody":
    "This item matches something you already have. Update the existing record, skip this item, or deliberately create a second one.",
  "ing.dupMatchedOn": "matched on",
  "ing.dupIdentical": "identical — nothing new to add",
  "ing.dupUpdate": "Update this one",
  "ing.dupWillUpdate": "✓ Will update",
  "ing.dupSkip": "Skip as duplicate",
  "ing.dupCreateNew": "Create a separate record",
  "ing.dupCreatingNew": "⚠ Creating a separate record",
  "ing.dupBlocked": "Decide what to do about the duplicate first.",
  "ing.dupStatus": "Duplicate",
  "ing.publishUpdate": "Approve & update",
  "ing.dupQueue": "Also staged in this queue",
  "ing.selectAll": "Select all",
  "ing.allKinds": "All",
  "ing.selectAllKind": "Select all {kind}",
  "ing.nSelected": "{n} selected",
  "ing.bulkApprove": "Approve selected",
  "ing.bulkRunning": "Publishing…",
  "ing.bulkConfirm": "Publish {n} item(s) to the library?",
  "ing.bulkDone": "Published {n} of {total} item(s).",
  "ing.dupWhatNext":
    "{n} held back as duplicates — what should happen to them?",
  "ing.dupReplace": "Replace existing",
  "ing.dupReplaceQ": "Overwrite the existing records?",
  "ing.dupBulkSkip": "Skip (keep existing)",
  "ing.dupSkipQ": "Close these as duplicates?",
  "ing.dupDelete": "Remove from queue",
  "ing.dupDeleteQ": "Delete these staged items?",
  "ing.bulkHeldTitle": "Held back — these were not published:",
  "ing.bulkHeldDup": "already in your library as",
  "plan.equipment": "Equipment used",
  "plan.anyEquipment": "Any / not specified",
  "plan.layer": "Device layer",
  "plan.noLayer": "Not layer-specific",
  "insp.layer": "Device layer",
  "insp.noLayer": "Not layer-specific",
  "insp.layerHint":
    "Which layer of the cell stack this step builds — shown on the flow card and used to group the test plan.",
  "lib.defaultLayer": "Default layer",
  "team.owner": "Owner",
  "team.team": "Team",
  "team.firstIsResponsible":
    "— first person is responsible for completing the work",
  "team.lead": "responsible",
  "team.searchFirst": "Search a person…",
  "team.searchMore": "Add participant…",
  "team.dragHint": "Drag to reorder — first is responsible",
  "team.remove": "Remove",
  "team.noMatch": "No matching person.",
  "org.title": "Organization",
  "org.subtitle":
    "settings, people, roles and who is responsible for each part of the lab",
  "org.settings": "Organization settings",
  "org.name": "Organization name",
  "org.number": "Organization number",
  "org.numberHint": "used in every experiment code — cannot be changed",
  "org.responsible": "Responsible people",
  "org.responsibleHint":
    "Assign who maintains each part of the library. Several people can share a responsibility; admins always have all of them.",
  "org.stewMaterials": "Materials",
  "org.stewMaterialsHint":
    "adds and edits materials, categories and supplier/batch data",
  "org.stewEquipment": "Equipment",
  "org.stewEquipmentHint": "adds and edits machines and their parameter sets",
  "org.stewFacilities": "Facilities",
  "org.stewFacilitiesHint": "manages locations and lab environments",
  "org.stewRecipes": "Recipes",
  "org.stewRecipesHint":
    "views approved recipe contents; does not grant editing",
  "org.recipeSteward": "Recipe steward",
  "org.stewDevices": "Tablets",
  "org.stewDevicesHint":
    "registers shared tablets, hands out setup links, revokes them",
  "org.recipeStewardHint": "views, edits, archives and approves recipes",
  "review.materials": "Material edit suggestions",
  "review.submit": "Submit for approval",
  "review.pending": "Pending approval",
  "review.approved": "Approved",
  "review.rejected": "Rejected",
  "review.approve": "Approve",
  "review.reject": "Reject",
  "review.failed":
    "Could not complete this action. Refresh and check permissions; a changed material needs a fresh suggestion.",
  "review.savedOnly": "Review applies to saved contents. Save any edits first.",
  "org.noneAssigned": "Nobody assigned yet",
  "org.adminImplicit": "Admins hold every responsibility automatically.",
  "org.you": "you",
  "org.manage": "Organization (admin)",
  "orgs.title": "Organizations",
  "orgs.subtitle":
    "Trusted organizations on this platform — approve onboarding requests and manage email domains",
  "orgs.manage": "Organizations (platform admin)",
  "orgs.name": "Organization",
  "orgs.admin": "Designated admin",
  "orgs.domains": "Email domains",
  "orgs.members": "Members",
  "orgs.status": "Status",
  "orgs.active": "Trusted",
  "orgs.pending": "Pending approval",
  "orgs.approve": "Approve",
  "orgs.reject": "Reject",
  "orgs.rejectQ": "Reject & remove?",
  "orgs.editDomains": "Click to edit domains",
  "orgs.invite": "Invite an organization",
  "orgs.inviteHint":
    "Generate a one-time link and send it to the organization's representative. They register their organization and its admin account; it appears above as pending until you approve it.",
  "orgs.generateLink": "Generate invite link",
  "orgs.copy": "Copy",
  "orgs.copied": "Copied ✓",
  "orgs.linkTtl": "The link is valid for 7 days and can be used once.",
  "onboard.title": "Register your organization",
  "onboard.subtitle":
    "You were invited to join the Pheno Lab Data Platform as a trusted organization.",
  "onboard.orgName": "Organization name",
  "onboard.domains": "Email domains",
  "onboard.domainsHint":
    "Comma-separated. Only emails on these domains can register into your organization.",
  "onboard.adminSection": "Designated admin account",
  "onboard.adminEmail": "Admin email (must be on one of the domains above)",
  "onboard.submit": "Submit for approval",
  "onboard.done":
    "Submitted. Your organization is pending approval — you'll be able to sign in once the platform admin approves it.",
  "onboard.badToken":
    "This invite link is invalid, expired, or already used. Ask the platform admin for a new one.",
  "onboard.emailDomain":
    "The admin email must be on one of the listed domains.",
  "onboard.badInput":
    "Check the organization name, domains, email, and use a password of at least 8 characters.",
  "cap.confirmFor": "Confirm for",
  "cap.capturedSamples": "Captured",
  "cap.simCode": "Sample code",
  "cap.applyToAll": "Apply to all",
  "cap.trashWhy":
    "What went wrong with this substrate? (recorded on the sample; add photos on the step card)",
  "cap.scanPolicy": "Displayed value (multiple scans):",
  "cap.policy.BEST": "Best",
  "cap.policy.MIN": "Minimum",
  "cap.policy.AVERAGE": "Average",
  "cap.policy.MEDIAN": "Median",
  "cap.perSampleHint": "Results are recorded one sample at a time.",
  "cap.overwriteQ": "This will overwrite the existing capture for:",
  "cap.overwrite": "Overwrite",
  "cap.addPhoto": "Add photo",
  "cap.photos": "Photos",
  "cap.noPhotos": "No photos yet — add one from the camera or gallery.",
  "cap.photoOf": "Photo",
  "cap.deletePhotoQ": "Delete photo?",
  "cap.confirmDeletePhoto": "Confirm delete photo",

  // Results comparison
  "res.title": "Results",
  "res.compare": "Results comparison",
  "res.subtitle": "measured results side by side across variation groups",
  "res.group": "Group",
  "res.mean": "mean",
  "res.noResults":
    "No results captured yet — record them in the capture portal.",
  "res.variables": "Tested variables",
  "res.samples": "Samples",
  "di.title": "AI data analysis — ask the whole experiment history",
  "di.hint":
    "Answers use every experiment you can see (test experiments excluded) and cite experiment codes. Takes about 30–60 seconds.",
  "di.ph":
    "e.g. Spin-coated vs blade-coated SAM — which performed better, and with which HTL?",
  "di.run": "Analyze",
  "di.running": "Analyzing…",
  "di.error": "Analysis failed or no AI provider is configured — try again.",
  "di.footer":
    "Based on {n} experiments · {model} · AI-generated, verify against the records",
  "res.dist": "Distributions by group",
  "res.distHint":
    "box = middle half · line = median · △ = mean · dots = samples",
  "res.open": "View results",
  "res.run": "Run",
  "rep.title": "Report",
  "rep.subtitle": "Experiment report",
  "rep.print": "Print / PDF",
  "labels.title": "Sample labels",
  "labels.hint":
    "A4 label stock, 21 per sheet (70 × 42.3 mm). Print at 100% scale with margins set to None.",
  "labels.empty": "This experiment has no samples yet.",
  "scan.button": "Scan label",
  "scan.hint": "Point the camera at a sample label QR code",
  "scan.fail": "Camera unavailable — check the camera permission and retry.",
  "scan.cancel": "Cancel",
  "rep.method": "Scientific method",
  "rep.plan": "Test plan",
  "rep.flow": "Process flow",
  "rep.charPlan": "Characterization",
  "rep.results": "Results",
  "rep.team": "Team",
  "rep.created": "Created",
  "rep.completed": "Status",
  "rep.materials": "Materials",
  "rep.equipment": "Equipment",
  "rep.environment": "Environment",
  "rep.parameters": "Parameters",
  "rep.deviations": "Captured deviations & notes",
  "rep.noDeviations": "No deviations from plan were recorded.",
  "rep.generated": "Generated by Pheno Lab Data Platform",

  // Registration
  "reg.title": "Create your account",
  "reg.subtitle":
    "Registration uses a one-time passcode sent for your organization email.",
  "reg.email": "Organization email",
  "reg.request": "Send passcode",
  "reg.codeSent":
    "A one-time passcode was generated. Enter it below — if you did not receive it, ask your admin (codes are visible in Users → Pending registrations).",
  "reg.codeEmailed":
    "A one-time passcode was sent to your email. Enter it below — check your spam folder if it doesn't arrive within a minute.",
  "reg.code": "One-time passcode",
  "reg.name": "Your name",
  "reg.password": "Choose a password",
  "reg.password2": "Confirm password",
  "reg.submit": "Verify & create account",
  "reg.done":
    "Registration submitted — an admin will approve your account shortly; you can sign in once it's confirmed.",
  "reg.badDomain":
    "This email domain is not registered to any organization. Ask your admin to add it.",
  "reg.badCode": "Invalid or expired passcode.",
  "reg.exists": "An account with this email already exists.",
  "reg.link": "New here? Register with your organization email",
  "reg.backToLogin": "Back to sign in",
  "reset.title": "Reset your password",
  "reset.subtitle":
    "Enter your account email and we will issue a one-time passcode.",
  "reset.request": "Send reset code",
  "reset.codeEmailed":
    "If this email has an account, a passcode was sent to it. Enter it below — check your spam folder if it does not arrive.",
  "reset.codeSent":
    "If this email has an account, a passcode was generated. Ask your admin for it (codes are visible in Users → Pending codes).",
  "reset.password": "New password",
  "reset.submit": "Set new password",
  "reset.done": "Password updated — you can sign in with it now.",
  "login.forgot": "Forgot your password?",
  "users.resetPw": "Reset password",
  "users.resetConfirm": "Set this password",
  "users.resetDone": "Password set — copy it now; it will not be shown again.",
  "users.codeRegister": "register",
  "users.codeReset": "reset",

  // User management
  "users.title": "Users",
  "users.subtitle": "manage your organization's accounts and access",
  "users.name": "Name",
  "users.email": "Email",
  "users.role": "Role",
  "users.team": "Team",
  "users.ungrouped": "— Ungrouped —",
  "users.active": "Active",
  "users.joined": "Joined",
  "users.deactivate": "Deactivate",
  "users.activate": "Activate",
  "users.domains": "Allowed registration domains",
  "users.domainsHint":
    "Only emails on these domains can register into this organization. Comma-separated, e.g. ultiprice.com, pheno.lab",
  "users.saveDomains": "Save domains",
  "users.pending": "Pending registrations",
  "users.pendingHint":
    "Active one-time passcodes — relay the code to the person registering if email delivery is not configured.",
  "users.noPending": "No pending registrations.",
  "users.expires": "expires",
  "users.manage": "Manage users (admin)",

  // System / storage
  "sys.title": "System & storage",
  "sys.subtitle": "where your data lives, how much space it uses, and backups",
  "sys.database": "Database",
  "sys.dbLocation": "Location",
  "sys.dbName": "Database name",
  "sys.dbSize": "Database size",
  "sys.uploads": "Uploads (photos & screenshots)",
  "sys.disk": "Disk",
  "sys.diskFree": "free of",
  "sys.tables": "Largest tables",
  "sys.backups": "Backups",
  "sys.backupNow": "Back up now",
  "sys.backupDone": "Backup created.",
  "sys.noBackups": "No backups yet.",
  "sys.backupHint":
    "Backups are pg_dump snapshots in the backups/ folder, rotated automatically (last 30 kept). A daily scheduled backup is installed on this machine.",
  "sys.backupExternalHint":
    "Database backups are managed and restored on the external PostgreSQL server.",
  "sys.migrateHint":
    "To move storage later (NAS/cloud), point DATABASE_URL at the new PostgreSQL host and restore the latest backup — no app changes needed.",
  "sys.manage": "System & storage (admin)",

  // Instrument data (J-V auto-ingest)
  "inst.title": "Instrument data",
  "inst.subtitle": "measurements pushed automatically by the J-V simulators",
  "inst.nav": "Instruments",
  "inst.manage": "Instrument data & uploaders",
  "inst.rigs": "Connected instruments",
  "inst.none": "No instrument has been registered yet.",
  "inst.lastSeen": "Last seen",
  "inst.never": "never",
  "inst.online": "online",
  "inst.offline": "not reporting",
  "inst.host": "Computer",
  "inst.watching": "Watching",
  "inst.inbox": "Incoming measurements",
  "inst.ownedBy": "for",
  "inst.selectAll": "Select all",
  "inst.nSelected": "{n} selected",
  "inst.handTo": "Hand to…",
  "inst.handedOver": "{n} measurement(s) handed to {name}",
  "inst.matched": "Matched",
  "inst.unmatched": "Needs attention",
  "inst.ignored": "Ignored",
  "inst.serial": "Serial typed on the instrument",
  "inst.sample": "Sample",
  "inst.measuredAt": "Measured",
  "inst.scan": "Scan",
  "inst.instrument": "Instrument",
  "inst.emptyInbox": "Nothing has arrived yet.",
  "inst.emptyUnmatched": "Everything that arrived has been matched.",
  "inst.rematch": "Re-check unmatched",
  "inst.rematching": "Re-checking…",
  "inst.rematchDone": "{matched} of {considered} now matched.",
  "inst.assign": "Attach to…",
  "inst.detach": "Detach",
  "inst.ignore": "Ignore",
  "inst.pickSample": "Choose a sample",
  "inst.autoHint":
    "Files arrive on their own — the agent on each simulator PC pushes every new measurement. Unmatched ones are re-checked automatically every few minutes, because a scan often arrives before the experiment that explains it exists.",

  // Per-experiment J-V pull
  "jv.title": "J-V files",
  "jv.pull": "Pull J-V files",
  "jv.pulling": "Pulling…",
  "jv.pulled": "{matched} newly matched · {files} file(s) attached",
  "jv.noneYet": "No J-V files have been matched to this experiment yet.",
  "jv.samples": "Samples",
  "jv.scansOne": "1 scan",
  "jv.scansMany": "{n} scans",
  "jv.noScans": "no file yet",
  "jv.best": "best",
  "jv.candidates": "Unmatched files from the last 30 days",
  "jv.candidatesHint":
    "If an operator mistyped the serial, attach the file to the right sample here.",
  "jv.attach": "Attach",
  "jv.attached": "Attached files",
  "jv.hint":
    "Operators type the sample ID into the instrument (e.g. {example}). Anything that matches lands here on its own.",
  "jv.serials": "Serials to type on the instrument",
  "jv.serialsHint":
    "Type this into the instrument's Serial NO. / Sample name field. Add the pixel after it, e.g. {example}-2.",
  "jv.copyAll": "Copy all",
  "jv.copied": "Copied",
  "jv.alias": "Also answers to",
  "jv.aliasAdd": "add another serial",
  "jv.aliasSaved": "Serial added",
  "kiosk.title": "Shared tablets",
  "kiosk.subtitle":
    "Registered lab tablets run in kiosk mode: sign in with a tap-to-pick name, auto sign-out after 1 hour idle, one user at a time, and no admin pages.",
  "kiosk.labelPh": "e.g. Tablet 3 · A2-202 evaporation",
  "kiosk.nameField": "Name this tablet",
  "kiosk.nameHint":
    "Pick a name that says which tablet this is and where it lives — the admin's list shows exactly this.",
  "kiosk.pendingName": "(unregistered — waiting for a technician to name it)",
  "kiosk.create": "New registration link",
  "kiosk.addHint":
    "The link is copied automatically — send it to a technician, who opens it on the tablet, names the device and confirms.",
  "kiosk.none": "No shared tablets registered yet.",
  "kiosk.unclaimed": "Setup link not yet opened on the device",
  "kiosk.inUse": "{name} is signed in · active {ago} ago",
  "kiosk.idle": "Nobody signed in · last activity {ago} ago",
  "kiosk.revoked": "Revoked — sessions on this device are blocked",
  "kiosk.copyLink": "Copy setup link",
  "kiosk.copied": "Copied",
  "kiosk.revoke": "Revoke",
  "kiosk.restore": "Restore",
  "kiosk.rules":
    "Kiosk sessions sign out after 1 hour idle and 12 hours at most; a new sign-in replaces whoever forgot to sign out.",
  "kiosk.manage": "Shared tablets",
  "kiosk.claimTitle": "Register this browser as a shared tablet",
  "kiosk.claimHint":
    "Only confirm on the lab tablet itself — never on a personal phone. This browser will then sign users out automatically.",
  "kiosk.claimConfirm": "Confirm — this is the lab tablet",
  "kiosk.claimInvalid": "This setup link is invalid or already used.",
  "kiosk.claimDone": "Tablet registered. Sign in below to start.",
  "kiosk.loginHint":
    "Shared tablet: tap your name, enter your password. You will be signed out automatically after 1 hour of inactivity.",
  "kiosk.badgeHint":
    "Shared tablet session — signs out automatically when idle.",
  "kiosk.idleTitle": "Still there?",
  "kiosk.idleBody":
    "No activity — signing out in {s} s to protect your account.",
  "kiosk.idleStay": "Keep working",
  "nfc.title": "NFC work badge",
  "nfc.hint":
    "Bind your badge once, then sign in on any shared lab tablet by tapping it — no typing. Binding works in Chrome on Android (do it on the lab tablet after a normal sign-in).",
  "nfc.bound": "Badge bound to this account",
  "nfc.bind": "Bind my badge",
  "nfc.rebind": "Bind a new badge",
  "nfc.unbind": "Unbind",
  "nfc.unbound": "Badge unbound.",
  "nfc.tapNow": "Hold your badge against the back of the device…",
  "nfc.holdStill": "Keep the badge still — writing…",
  "nfc.bindDone": "Badge bound. From now on, tap to sign in on shared tablets.",
  "nfc.bindDoneUidOnly":
    "Badge bound (chip is write-protected — bound by serial number only).",
  "nfc.bindFailed": "Binding failed — try again.",
  "nfc.timeout": "No badge detected — try again and hold the card steady.",
  "nfc.unsupported":
    "This browser cannot read NFC. Use Chrome on an Android tablet or phone to bind.",
  "nfc.loginBtn": "Tap badge to sign in",
  "nfc.loginTap": "Hold your badge against the back of the tablet…",
  "nfc.loginChecking": "Checking badge…",
  "nfc.loginFailed":
    "Badge not recognized. Bind it on your profile page first, or sign in with your password.",
  "nfc.loginRetry": "Try again",
  "users.editIdentity": "Edit name and email",
  "users.cancel": "Cancel",
  "login.switchUser": "Not you?",
  "login.pending":
    "Your account is waiting for admin approval — you'll be able to sign in once it's confirmed.",
  "appr.title": "Registrations awaiting approval",
  "appr.hint":
    "New sign-ups go live only after you approve them. Fix the name/handle/email styling here, and claim their legacy imported data in the same step if they have any.",
  "appr.registered": "Registered as",
  "appr.handle": "Handle",
  "appr.legacy": "Legacy data",
  "appr.legacyHint":
    "If this person's pre-platform experiments were imported, pick their dataset — it becomes theirs the moment you approve.",
  "appr.noLegacy": "No legacy data",
  "appr.expCount": "{n} experiments",
  "appr.suggested": "name match",
  "appr.approve": "Approve",
  "appr.reject": "Reject",
  "req.ownedBy": "Experiment by {name}",
  "req.explain":
    "This experiment belongs to a colleague. Ask for access — once the owner (or a manager) approves, this page opens normally and you join as a member.",
  "req.declined": "Your previous request was declined. You can ask again.",
  "req.messagePh": "Why do you need access? (optional)",
  "req.send": "Request access",
  "req.sending": "Sending…",
  "req.pending": "Request sent — waiting for {name} to approve.",
  "req.asksAccess": "requests access to this experiment",
  "req.approve": "Approve",
  "req.decline": "Decline",
  "notif.draft_idle_warning":
    "will archive your idle draft in 3 days — open and edit it to keep it",
  "notif.draft_idle_archived":
    "archived your idle draft — open it to renew in one click",
  "idle.warnChip": "Idle · archives in 3 days",
  "idle.warnHint":
    "No edits for 7 days. Any edit keeps it; otherwise it is archived automatically.",
  "idle.archivedChip": "Archived for inactivity",
  "idle.archivedHint":
    "Archived automatically after 10 idle days. Renew to continue.",
  "idle.renew": "Renew draft",
  "idle.archivedBanner":
    "This draft was archived for inactivity. Nothing was deleted.",
  "idle.warnBanner":
    "Idle for 7 days — it will be archived automatically in 3 days unless something changes. Any edit counts.",
  "notif.title": "Notifications",
  "notif.loading": "Loading…",
  "notif.empty": "Nothing yet — colleague interactions will show up here.",
  "notif.justNow": "just now",
  "notif.access_requested": "requests access to your experiment",
  "notif.access_approved": "approved your access request",
  "notif.access_declined": "declined your access request",
  "notif.feedback_approved": "approved your feedback",
  "notif.feedback_rejected": "rejected your feedback",
  "notif.feedback_implemented":
    "implemented your feedback — open your profile to test and confirm",
  "notif.feedback_commented": "commented on your feedback",
  "notif.feedback_verified": "confirmed your implementation works",
  "notif.feedback_reopened": "sent an implemented feedback back",
  "notif.assigned": "assigned an experiment to you",
  "notif.member_added": "added you to an experiment",
  "notif.material_edit_requested": "submitted a material edit for review",
  "notif.material_edit_approved": "approved your material edit",
  "notif.material_edit_rejected": "rejected your material edit",
  "notif.recipe_approval_requested": "submitted a recipe for approval",
  "notif.recipe_approved": "approved your recipe",
  "notif.recipe_rejected": "rejected your recipe",
  "notif.experiment_commented": "commented on your experiment",
  "notif.mentioned": "mentioned you in a comment",
  "cmt.title": "Discussion",
  "cmt.empty": "No comments yet — type @ to bring someone in.",
  "cmt.placeholder": "Write a comment… @ mentions notify",
  "cmt.post": "Post",
  "cmt.delete": "Delete comment",
  "jvr.unlinked": "{n} instrument scan(s) not linked to any sample",
  "jvr.hint":
    "A mistyped sample ID on the instrument leaves its scan unlinked. Pick the sample each scan belongs to and link it — the values fill in immediately.",
  "jvr.pick": "Sample…",
  "jvr.suggested": "suggested",
  "jvr.link": "Link",
  "jvr.linked": "Linked — values filled in.",
  "jvr.others": "Scans that look like other experiments",
} as const;

export type TKey = keyof typeof en;

export const zh: Record<TKey, string> = {
  "app.name": "实验数据平台",
  "nav.dashboard": "仪表盘",
  "nav.experiments": "实验",
  "nav.library": "资源库",
  "nav.data": "数据",
  "nav.signout": "退出登录",
  "nav.back": "返回",
  "nav.profile": "个人资料",

  "login.email": "邮箱",
  "login.password": "密码",
  "login.signin": "登录",
  "login.signingin": "正在登录…",
  "login.invalid": "邮箱或密码错误。",

  "role.ADMIN": "管理员",
  "role.MANAGER": "经理",
  "role.TECHNICIAN": "技术员",
  "status.DRAFT": "草稿",
  "status.IN_LAB": "实验中",
  "status.REVIEW": "待审核",
  "status.COMPLETE": "已完成",
  "status.ARCHIVED": "已归档",

  "list.title": "实验",
  "list.experiments": "个实验",
  "list.experiment": "个实验",
  "list.assigned": "（分配给您的）",
  "list.involved": "（您创建或参与的）",
  "list.newPlaceholder": "新实验标题…",
  "list.sortHint": "点击排序，再点反向",
  "list.filterHint": "筛选…",
  "list.all": "全部",
  "list.clearFilters": "清除",
  "list.create": "创建",
  "list.code": "编号",
  "list.titleCol": "标题",
  "list.status": "状态",
  "list.createdBy": "创建人",
  "list.samples": "样品",
  "list.steps": "步骤",
  "list.labels": "标签",
  "list.updated": "更新时间",
  "list.emptyStaff": "暂无实验。创建一个实验来设计您的第一个工艺流程。",
  "list.emptyTech": "暂无分配给您的实验。请联系经理为您授权。",

  "dash.title": "仪表盘",
  "dash.titleTech": "我的任务",
  "dash.subtitle": "所有进行中实验的进度",
  "dash.subtitleTech": "分配给您的实验",
  "dash.kanban": "看板",
  "dash.cards": "卡片",
  "dash.list": "列表",
  "dash.search": "搜索实验…",
  "dash.capture": "采集数据",
  "dash.duplicate": "复制实验",
  "dash.duplicated": "实验已复制。",
  "dash.new": "新建实验",
  "dash.campaign": "专题",
  "dash.project": "项目组",
  "dash.allProjects": "全部项目组",
  "dash.noProject": "未指定项目组",
  "dash.needTitle":
    "送交实验室前请填写有意义的实验标题——「Untitled experiment」既无法检索，也无法生成报告。",
  "dash.needProject": "送交实验室前请在设置中选择所属项目组。",
  "dash.empty": "暂无内容。",
  "dash.open": "打开",
  "dash.dragHint": "拖动卡片到其他列可更改状态。",

  "designer.settings": "设置",
  "designer.saveDraft": "保存草稿",
  "designer.sendToLab": "送入实验室",
  "designer.readonly": "只读",
  "designer.processLibrary": "工艺库",
  "designer.railHintEdit": "点击工艺将其添加到流程末尾。拖动卡片可调整顺序。",
  "designer.railHintView": "本实验室可用的工艺类型。",
  "designer.characterization": "表征",
  "designer.processFlow": "工艺流程",
  "designer.flowHint": "被测变量所在步骤高亮显示",
  "designer.dragHint": "拖动卡片调整顺序",
  "designer.charPlan": "表征方案",
  "designer.charHint": "在工艺完成后进行",
  "designer.emptySteps":
    "暂无工艺步骤。请从左侧工艺库添加第一步——通常从基片清洗开始。",
  "designer.emptyChars": "暂无表征计划。请从左侧添加测试项目。",
  "designer.saved": "所有更改已保存",
  "designer.saving": "保存中…",
  "designer.saveFailed": "保存失败——请重试上一次编辑",
  "designer.samples": "个样品",
  "designer.processSteps": "个工艺步骤",
  "designer.characterizations": "项表征",
  "designer.selectHint": "点击流程中的卡片进行查看和编辑。",
  "designer.selectHintEdit":
    "使用实验方案定义本实验的变量——被测步骤将以绿色高亮。",

  "sci.observation": "观察",
  "sci.problem": "问题",
  "sci.hypothesis": "假设",
  "sci.conclusion": "结论",
  "sci.observationPh": "是什么观察结果促使您开展本实验？",
  "sci.problemPh": "本实验要回答什么科学问题？",
  "sci.hypothesisPh": "您预期的结果是什么？依据是什么？",
  "sci.conclusionLocked": "实验标记为已完成后解锁。",
  "sci.aiSummary": "AI 辅助总结",
  "sci.aiHint": "撰写结论前，让 AI 分析完整的实验记录。",
  "sci.aiGenerate": "生成",
  "sci.aiRegenerate": "重新生成",
  "sci.aiBusy": "分析中…",
  "sci.aiBackground": "后台运行——可离开此页面，稍后回来查看。",
  "sci.aiLocked": "实验完成后与结论一起解锁。",
  "dash.viewMore": "查看更多（{n}）",
  "activity.title": "动态",
  "activity.online": "当前在线",
  "activity.nobody": "当前无人在线。",
  "activity.empty": "暂无活动记录。",
  "activity.collapse": "收起",
  "act.created": "创建了",
  "act.updated": "更新了",
  "act.deleted": "删除了",
  "act.published": "发布了",
  "act.imported": "导入了",
  "act.duplicated": "复制了",
  "act.aiSummary": "生成了 AI 总结：",
  "act.changed": "修改了",
  "sci.aiFailed":
    "未配置 AI 模型或调用失败——请重试或在个人资料中检查 AI 服务。",
  "sci.aiDisclaimer": "AI 生成内容，引用前请核对数据",

  "plan.title": "实验方案",
  "plan.question": "您要测试哪些变量？它们属于哪个工艺？分几组？对照组是哪组？",
  "plan.testing": "测试变量：",
  "plan.in": "所属工艺：",
  "plan.none": "尚未定义变量——本实验要测试什么？",
  "plan.edit": "编辑实验方案",
  "plan.define": "定义实验方案",
  "plan.variableType": "类型",
  "plan.variable": "变量",
  "plan.parameterKind": "工艺参数",
  "plan.materialKind": "材料",
  "plan.process": "工艺",
  "plan.parameterTested": "被测参数",
  "plan.selectParameter": "选择参数…",
  "plan.customParameter": "自定义参数…",
  "plan.materialSlot": "材料位名称",
  "plan.unit": "单位",
  "plan.addVariable": "添加变量",
  "plan.group": "组别",
  "plan.extras": "备用",
  "plan.errors": "报废 / 问题",
  "plan.dropHere": "拖拽基片到此",
  "plan.substrates": "基片批次",
  "plan.substrateCount": "准备基片数量",
  "plan.substrateMaterial": "基片材料",
  "plan.substrateHint":
    "将每片基片拖入对应组。备用基片放在“备用”，出错的拖入“报废”，随时用备用片替换，编号不变。",
  "plan.regroup": "调整基片分组",
  "plan.samples": "样品数",
  "plan.control": "对照组",
  "plan.controlWord": "对照",
  "plan.selectMaterial": "选择材料…",
  "plan.addMaterial": "＋ 新建材料…",
  "plan.applyAll": "应用到所有组",
  "plan.addGroup": "添加组",
  "plan.total": "样品总数：",
  "plan.autoCalc": "（自动计算）",
  "plan.applyNote":
    "应用后将重新生成样品，并在对应工艺步骤上标记被测变量（如无该步骤则自动添加）。从方案中移除的变量会同步从流程中清除。",
  "plan.cancel": "取消",
  "plan.apply": "应用实验方案",

  "insp.step": "步骤",
  "insp.char": "表征",
  "insp.preset": "预设——常用配置",
  "insp.applyPreset": "应用预设…",
  "insp.used": "已用",
  "insp.presetApplied": "已应用预设——保存更改后生效。",
  "insp.stepName": "步骤名称",
  "insp.name": "名称",
  "insp.equipment": "设备",
  "insp.instrument": "仪器",
  "insp.noEquipment": "无设备",
  "insp.noInstrument": "无仪器",
  "insp.equipmentHintEmpty": "该工艺（{process}）暂无设备——请在资源库中添加。",
  "insp.equipmentHint": "仅可选择该工艺下的设备。选择设备后将载入其参数配置。",
  "insp.parameters": "工艺参数",
  "insp.parameter": "参数",
  "insp.value": "数值",
  "insp.unit": "单位",
  "insp.addParameter": "添加参数",
  "insp.vary": "按组设置不同值",
  "insp.varied": "被测变量——按组变化",
  "insp.removeVariation": "移除变量设置",
  "insp.environment": "环境",
  "insp.noEnvironment": "未指定环境",
  "insp.materials": "沉积材料",
  "insp.noMaterial": "本步骤不沉积材料。",
  "insp.selectMaterial": "选择材料…",
  "insp.addMaterial": "添加材料",
  "insp.addMaterialOpt": "＋ 新建材料…",
  "insp.amountPh": "用量",
  "insp.amountMixPh": "如 60 mol%",
  "insp.mixHint": "同一工艺中使用多种材料时，请注明各自比例（如 mol%）。",
  "insp.materialFilterHint": "仅显示归类于 {process} 工艺的材料。",
  "insp.settings": "测试条件",
  "insp.setting": "条件项",
  "insp.addSetting": "添加条件",
  "insp.sampleScope": "样品范围",
  "insp.allSamples": "全部样品",
  "insp.perGroup": "每组一个样品",
  "insp.notes": "备注",
  "insp.save": "保存更改",
  "insp.cancel": "取消更改",
  "insp.saveAsPreset": "保存为预设",
  "insp.presetNamePrompt": "预设名称：",
  "insp.newMaterialPrompt": "新材料名称：",

  "card.noEquipment": "未指定设备",
  "card.noInstrument": "未指定仪器",
  "card.testing": "测试变量：",
  "card.allSamples": "全部样品",
  "card.perGroup": "每组 1 个",
  "card.delete": "删除",
  "card.deleteQ": "删除？",

  "set.title": "实验设置",
  "set.expTitle": "标题",
  "set.status": "状态",
  "set.statusHint":
    "方案就绪后将状态从「草稿」改为「实验中」以交给技术员执行。结论在「已完成」后解锁。",
  "set.participants": "参与人员",
  "set.creator": "创建人",
  "set.remove": "移除",
  "set.grant": "＋ 授权给…",
  "set.autoLabels": "自动生成的标签",
  "set.autoLabelsHint": "根据实验所用的工艺、设备、材料和变量自动生成。",
  "set.project": "项目组",
  "set.projectNone": "— 未指定 —",
  "set.projectNew": "新建项目组",
  "set.projectAdd": "添加",
  "set.projectHint":
    "本实验所属的项目组，默认为你自己所在的组。送交实验室前必填——它用于看板归类和筛选，不会限制分析可比较的实验范围。",
  "set.campaign": "专题 / 实验组",
  "set.campaignHint":
    "可选——将相关实验归入同一个大专题，如「Q3 退火优化」。可在看板中搜索。",
  "nav.analysis": "分析",
  "an.askTab": "提问",
  "an.manualTab": "单条件对比",
  "an.askSubtitle":
    "直接提问。系统检索相关实验、计算对比数据，再由模型综合解读——多个因素一并考虑，而不是一次只看一个。",
  "an.askTitle": "向实验室的历史提问",
  "an.askHint":
    "任何关于实验室做过什么的问题：一种材料、一道工艺、一种组合、一个趋势。检索范围是实验室全部实验，无需预先选择。回答需要一到三分钟，结果对所有人可见。",
  "an.askPlaceholder": "例如：刮涂 HTL 是否优于旋涂？是否取决于所用的 SAM？",
  "an.askShortcut": "⌘/Ctrl + Enter 提问",
  "an.askButton": "提问",
  "an.noHistory": "还没有人提过问题。",
  "an.running": "正在读取数据…",
  "an.runFailed": "未能完成",
  "an.basedOn": "{experiments} 个实验 · {samples} 个样品",
  "an.experimentsRead": "读取的实验",
  "an.terms": "检索词",
  "an.recipeSource": "来自配方差异",
  "an.title": "跨实验分析",
  "an.subtitle":
    "把实验室的全部实验按各自变化的条件汇总。选择一个条件，即可看到它的不同取值在各批次之间的表现。",
  "an.condition": "变量条件",
  "an.pickCondition": "— 选择条件 —",
  "an.metric": "指标",
  "an.from": "起始",
  "an.to": "截止",
  "an.filter": "筛选",
  "an.filterHint": "材料、工艺、编号或标题…",
  "an.reset": "清除",
  "an.scopeLine": "{experiments} 个实验 · {samples} 个有数据的样品",
  "an.truncated": "当前仅显示最近 400 个实验。缩小时间范围可获得完整结果。",
  "an.emptyScope": "该范围内暂无测试数据。",
  "an.emptyCondition":
    "请在上方选择条件。列表中是实验室做过两种以上取值并且有测试数据的全部参数。",
  "an.pooled": "汇总对比",
  "an.pooledHint": "范围内所有实验中，在每个取值下测得的全部样品。",
  "an.value": "取值",
  "an.samples": "样品数",
  "an.experiments": "实验数",
  "an.mean": "平均",
  "an.median": "中位数",
  "an.best": "最佳",
  "an.sd": "标准差",
  "an.perBatch": "逐批次对比",
  "an.perBatchHint":
    "在每个实验内部做同样的对比——此时其余工艺条件是一致的。能在这里重复出现的结论，比汇总平均值可靠得多。",
  "an.winner": "本批最佳",
  "an.wins": "获胜次数",
  "an.winsHint": "每个取值在各自批次内胜出的次数。",
  "an.plots": "分布",
  "an.plotsHint": "箱体为四分位，须线为极差，三角为平均值，圆点为单个样品。",
  "an.anomalies": "异常提示",
  "an.export": "下载表格（CSV）",
  "an.caution":
    "跨批次汇总会把批次之间其他差异一并混入。请以逐批次对比作为更可靠的证据。",
  "an.ai": "AI 解读",
  "an.aiHint":
    "DeepSeek 读取上方已计算好的表格（不读取原始曲线），总结哪些结论可重复、哪些相互矛盾，以及下一步值得补做的条件。",
  "an.ask": "分析当前范围",
  "an.asking": "正在读取数据…",
  "an.aiEmpty": "请先选择一个条件，才有可解读的内容。",
  "an.aiFailed": "分析未能完成，请重试。",
  "an.aiStale": "范围已改变——请重新运行以获得对应的结果。",
  "an.generated": "生成于 {when} · {model}",
  "anom.champion": "最佳器件：{sample}（{group}），PCE {value}。",
  "anom.negative":
    "{sample}：{metric} 为负（{value} Ω）——属测量伪影，并非器件本身特性。该次扫描的 PCE/Voc/Jsc/FF 仍然有效。",
  "anom.outlier":
    "{sample}（{group} 组）：PCE {value} 超出本组正常范围（中位数 {median}）。",
  "proj.title": "项目组",
  "proj.hint":
    "按组织架构设置的项目组。每个人所在的组在「组织」页面设置，新建实验会默认归入自己的组。小组解散后可停用，它会从选择列表中隐藏，但不影响已有实验。",
  "proj.name": "项目组名称",
  "proj.add": "新增项目组",
  "proj.experiments": "个实验",
  "proj.active": "启用中",
  "proj.inactive": "已停用",
  "proj.activate": "重新启用",
  "proj.deactivate": "停用",
  "proj.rename": "重命名",
  "proj.save": "保存",
  "proj.cancel": "取消",
  "proj.empty": "暂无项目组。",
  "set.delete": "删除实验",
  "set.deleteQ": "确定删除本实验及其全部数据？",
  "set.deleteYes": "删除",
  "set.deleteNo": "取消",

  "lib.processes": "工艺",
  "lib.processesHint": "顶层结构——设备与材料归属于各工艺之下",
  "lib.characterization": "表征",
  "lib.charHint": "测试项目及其仪器",
  "lib.equipmentFor": "设备：",
  "lib.materialsFor": "材料：",
  "lib.paramsFor": "工艺参数：",
  "lib.paramsHint":
    "该工艺每个步骤记录的参数。每项参数在数据库中均标记为工艺参数。新步骤以此为初始参数；选择设备后由设备参数覆盖。",
  "lib.noEquipment": "该工艺下暂无设备。",
  "lib.noMaterials": "该工艺下暂无材料。",
  "lib.addEquipment": "添加设备",
  "lib.addMaterial": "添加材料",
  "lib.addProcess": "添加工艺",
  "lib.renameProcess": "重命名工艺",
  "lib.archiveProcess": "归档工艺",
  "lib.restoreProcess": "恢复工艺",
  "lib.nickname": "设备昵称（同事习惯的叫法）",
  "lib.nicknamePh": "如：小蒸发台",
  "lib.equipmentName": "设备名称",
  "lib.manufacturer": "制造商",
  "lib.modelNumber": "型号",
  "lib.assetTag": "资产编号",
  "lib.location": "位置",
  "lib.noLocation": "无位置",
  "lib.addLocation": "＋ 新增位置…",
  "lib.photo": "照片",
  "lib.uploadPhoto": "上传照片",
  "lib.replacePhoto": "更换照片",
  "lib.removePhoto": "移除",
  "lib.uploading": "上传中…",
  "lib.envDetails": "环境说明",
  "lib.envDetailsHint":
    "描述环境本体——品牌、型号、腔体配置。上方的条件是每次实验需要记录的读数。",
  "lib.specSheets": "设备资料",
  "lib.specSheetsHint": "该设备的厂家原始文件——说明书、规格书、验收报告等。",
  "lib.addSpecSheet": "添加文件",
  "lib.noSpecSheets": "暂无附件",
  "lib.machineParams": "设备规格",
  "lib.machineParamsHint":
    "规格书上的参考值——仅在设备卡片上展示，不会填入工艺步骤。",
  "lib.workParams": "设备工艺参数（实验中使用）",
  "lib.workParamsHint":
    "技术员日常在该设备上实际调试的参数。工艺步骤或实验方案变量选择该设备后，自动带入并可选取的就是这里的定义。",
  "lib.materialName": "材料名称",
  "lib.composition": "成分 / 配方",
  "lib.supplier": "供应商",
  "lib.lotNumber": "批号",
  "lib.edit": "编辑",
  "lib.close": "收起",
  "lib.archive": "归档",
  "lib.archived": "已归档",
  "lib.unarchive": "恢复",
  "lib.restore": "恢复",
  "lib.cancel": "取消",
  "lib.saveChanges": "保存更改",
  "lib.saveParams": "保存参数",
  "lib.environments": "环境",
  "lib.environmentsHint": "实验室的环境类型及各环境记录的条件",
  "lib.envName": "环境名称",
  "lib.conditions": "记录条件",
  "lib.conditionsHint":
    "该环境需记录的条件——如手套箱的 O₂/H₂O 含量（ppm）与温度，或洁净室的洁净度与湿度。",
  "lib.addEnvironment": "添加环境",
  "lib.locations": "位置",
  "lib.locationsHint": "机构位置很少变动——设置一次，处处可选",
  "lib.addLocationBtn": "添加位置",
  "lib.newLocationPh": "新位置，如：A 栋——钙钛矿实验室",
  "lib.noLocations": "暂无位置。",
  "lib.presets": "预设",
  "lib.presetsHint": "点击预设可编辑——在设计器中通过「保存为预设」创建",
  "lib.presetName": "预设名称",
  "lib.paramValues": "参数值",
  "lib.presetSettings": "测试条件",
  "lib.noPresets": "暂无预设——在设计器中配置步骤后使用「保存为预设」。",
  "lib.stepKind": "工艺步骤",
  "lib.charKind": "表征",
  "lib.process": "工艺",
  "lib.defName": "名称",
  "lib.defUnit": "单位",
  "lib.defDefault": "默认值",
  "lib.add": "添加",
  "lib.equipmentCount": "台设备",
  "lib.materialsCount": "种材料",
  "lib.processName": "工艺名称",
  "lib.icon": "图标",

  "data.title": "数据表",
  "data.subtitle": "每个样品一行——参数已按变量组解析，带标签、可直接导出。",
  "data.filter": "筛选行…",
  "data.search": "搜索人员、材料、工艺、样品、配方……或直接提问",
  "data.showMatches": "展开匹配的实验",
  "data.hideMatches": "收起",
  "data.searchGo": "搜索",
  "data.pageOf": "第 {page} / {pages} 页",
  "data.expMatching": "{n} 个实验",
  "data.cells": "本页单元格",
  "data.rows": "行",
  "data.columns": "列",
  "data.export": "导出 CSV",
  "data.noRows": "无匹配行。",

  "profile.title": "个人资料",
  "profile.displayName": "显示名称",
  "profile.handle": "用户名",
  "profile.handlePh": "如 @mike",
  "profile.save": "保存资料",
  "profile.loginInfo": "登录信息",
  "profile.email": "邮箱",
  "profile.password": "密码",
  "profile.currentPassword": "当前密码",
  "profile.newPassword": "新密码",
  "profile.confirmPassword": "确认新密码",
  "profile.changePassword": "修改密码",
  "profile.passwordChanged": "密码已修改。",
  "profile.passwordMismatch": "两次输入的新密码不一致。",
  "profile.passwordWrong": "当前密码不正确。",
  "profile.organization": "所属机构",
  "profile.role": "角色",
  "profile.memberSince": "加入时间",
  "profile.language": "语言",
  "profile.langHint": "对您账户下的整个应用生效。",
  "profile.stats": "我的统计",
  "profile.statExperiments": "参与实验数",
  "profile.statCompleted": "已完成实验数",
  "profile.statSamples": "计划样品数",
  "profile.statDataPoints": "已保存数据点",
  "profile.statPresets": "创建预设数",
  "profile.statDisk": "服务器磁盘已用 · {used} / {total} GB",
  "tr.showOriginal": "原文",
  "tr.showTranslation": "译文",
  "tr.translate": "翻译",
  "tr.translating": "翻译中…",
  "profile.saved": "已保存。",

  "fb.title": "报告问题 / 提交反馈",
  "fb.kind": "类型",
  "fb.bug": "问题报告",
  "fb.feedback": "功能反馈",
  "fb.message": "发生了什么？或有什么可以改进？",
  "fb.messagePh":
    "请描述问题或建议。若是 Bug：您预期的行为是什么？实际发生了什么？",
  "fb.screenshot": "截图（可选）",
  "fb.attach": "附加截图",
  "fb.attached": "截图已附加",
  "fb.includeErrors": "自动附带最近的错误日志",
  "fb.submit": "提交",
  "fb.submitted": "感谢反馈——已记录。",
  "fb.adminTitle": "反馈与问题报告",
  "fb.adminHint": "全机构范围收集——可导出交由智能体修复",
  "fb.exportJson": "导出 JSON",
  "fb.open": "待处理",
  "fb.resolved": "已解决",
  "fb.markResolved": "标记已解决",
  "fb.reopen": "重新打开",
  "fb.none": "暂无反馈。",
  "nav.feedback": "反馈",
  "fb.boardTitle": "反馈",
  "fb.boardHint":
    "每条反馈只描述一个问题——写清情况、附上截图，并在这里跟进管理员的处理意见。",
  "fb.newItem": "提交问题或建议",
  "fb.titlePh": "简短标题，如“实验方案里材料下拉框显示不全”",
  "fb.addShot": "截图",
  "fb.st.open": "待处理",
  "fb.st.approved": "已批准",
  "fb.st.rejected": "已拒绝",
  "fb.st.implemented": "已实现",
  "fb.st.resolved": "已实现",
  "fb.st.verified": "已验收",
  "fb.st.reopened": "已退回",
  "fb.tab.open": "待处理",
  "fb.tab.approved": "已批准",
  "fb.tab.rejected": "已拒绝",
  "fb.tab.implemented": "已实现",
  "fb.tab.all": "全部",
  "fb.tab.verified": "已验收",
  "fb.verifyBanner": "有 {n} 条已实现的反馈等待你测试确认——点击查看实现说明。",
  "fb.implNote": "实现说明",
  "fb.implNotePh": "改了什么、怎么用——反馈人会按这个说明去验证。",
  "fb.implNoteRequired": "请先填写实现说明，反馈人需要据此验证。",
  "fb.disputeNote": "反馈人退回原因",
  "fb.verifyAsk": "该反馈已实现——请实际试用后确认效果。",
  "fb.autoOn": "无操作将自动通过：",
  "fb.verifyOk": "✓ 满意，通过",
  "fb.verifyNo": "仍有问题",
  "fb.verifyNoPh": "还有什么问题或缺什么？",
  "fb.verifySend": "退回",
  "fb.verifiedAuto": "7 天无异议，已自动通过",
  "fb.verifiedYou": "你已确认通过此更改",
  "fb.reopenedWait": "已退回——等待再次处理。",
  "fb.adminNote": "管理员意见",
  "fb.adminNotePh": "范围说明、优先级，或拒绝原因……",
  "fb.approve": "批准",
  "fb.reject": "拒绝",
  "fb.markImplemented": "标记已实现",
  "fb.saveNote": "保存意见",
  "fb.viewAll": "查看全部反馈（管理员）",
  "fb.errorLog": "错误日志",
  "fb.page": "页面",

  "cap.title": "数据采集",
  "cap.sample": "样品",
  "cap.progress": "进度",
  "cap.planned": "计划值",
  "cap.actual": "实际值",
  "cap.confirm": "确认本步骤",
  "cap.confirmed": "已采集",
  "cap.edit": "修改采集",
  "cap.note": "备注",
  "cap.notePh": "有异常吗？偏差、观察记录…",
  "cap.flag": "标记问题",
  "cap.flagged": "已标记",
  "cap.photo": "照片",
  "cap.photoAttached": "照片已附加",
  "cap.environment": "环境条件",
  "cap.steps": "工艺步骤",
  "cap.results": "表征结果",
  "cap.metric": "指标",
  "cap.value": "数值",
  "cap.addMetric": "添加指标",
  "cap.saveResult": "保存结果",
  "cap.resultSaved": "结果已保存",
  "cap.notInLab":
    "本实验尚未进入实验阶段。将状态设为「实验中」后即可开始采集数据。",
  "cap.open": "打开采集页面",
  "cap.allDone": "该样品的所有步骤均已采集。",
  "cap.doneOf": "已采集",
  "cap.stepOf": "步骤",
  "cap.of": "/",
  "cap.prev": "上一步",
  "cap.next": "下一步",
  "cap.swipeHint": "左右滑动切换步骤 · 点击圆点跳转",
  "cap.run": "批次",
  "cap.newRun": "开始新批次",
  "cap.newRunHint":
    "使用同一方案再次执行，记录全新的实际值与结果（如重复性验证批次）。",
  "cap.deleteRun": "删除当前批次",
  "cap.deleteRunQ": "删除批次 {run}？",
  "cap.confirmDeleteRun": "确认删除批次",
  "cap.keepOneRun": "至少保留一个批次",

  "portal.title": "实验录入门户",
  "portal.subtitle": "可进行数据采集的实验",
  "portal.captureNow": "点击开始采集",
  "portal.progress": "已采集",
  "portal.empty":
    "当前没有进行中的实验。实验状态设为「实验中」后会显示在这里。",
  "portal.toDesktop": "切换到完整工作台",
  "portal.install": "安装此应用",
  "portal.iosTitle": "将 Pheno Lab 添加到主屏幕",
  "portal.iosStep1": "点击浏览器工具栏中的分享按钮",
  "portal.iosStep2": "选择“添加到主屏幕”",
  "portal.iosHint": "图标将以全屏应用方式打开，显示 Pheno 标志。",
  "portal.toPortal": "录入门户",
  "portal.other": "其他实验",
  "cap.applyTo": "应用于",
  "cap.batchAll": "全部样品",
  "cap.batchGroup": "组",
  "cap.batchOne": "单个样品",
  "cap.batchHint":
    "点击组或单个样品选择记录范围——确认一次即应用到所选样品。含被测变量的步骤按组分别采集。",
  "cap.pickScope": "请在上方选择一个组或样品开始记录。",
  "cap.complete": "完成实验",
  "cap.completeHint": "数据已全部采集——将实验标记为已完成。",
  "cap.completeMissing": "项数据尚未采集——也可先完成实验，稍后补录。",
  "cap.completeQ": "确认标记为已完成？",
  "cap.completed": "实验已完成",
  "cap.viewReport": "查看报告",
  "designer.addStep": "添加步骤",
  "designer.addChar": "添加表征",
  "cap.clear": "清除采集",
  "cap.clearQ": "确认清除采集？",
  "users.add": "添加成员",
  "users.addHint":
    "直接创建登录账号——在验证码邮件尚未配置时尤其有用。请将账号密码告知同事，他们可在个人资料中自行修改密码。",
  "users.password": "密码",
  "users.generate": "生成密码",
  "users.create": "创建账号",
  "users.created": "账号已创建——请将以下凭据告知同事：",
  "users.emailTaken": "该邮箱已存在账号。",
  "users.badInput": "请检查邮箱地址，密码至少 8 位。",
  "users.pwShort": "密码至少需要 8 位。",
  "mat.title": "材料库",
  "mat.subtitle": "实验室材料百科——由材料管理员维护",
  "mat.search": "搜索材料（名称、分子式、CAS）…",
  "mat.readonlyHint":
    "所有成员都可新增材料。修改已有材料需提交建议，由材料负责人审批后生效。",
  "mat.add": "添加",
  "mat.new": "新材料",
  "mat.name": "名称",
  "mat.category": "类别",
  "mat.formula": "分子式 / 结构",
  "mat.smiles": "SMILES（有机分子）",
  "mat.smilesHint": "结构式记法——用于绘制二维结构图。无机盐与金属可留空。",
  "mat.structure": "分子结构",
  "mat.mw": "分子量 (g/mol)",
  "mat.purity": "纯度",
  "mat.vendor": "供应商",
  "mat.lot": "批次",
  "mat.props": "特性",
  "mat.propName": "特性名（如 HOMO (eV)）",
  "mat.addProp": "添加特性",
  "mat.notes": "备注",
  "mat.emptyCat": "该类别暂无材料。",
  "mat.noMatch": "未找到匹配材料。",
  "mat.categories": "类别管理",
  "mat.catNew": "新建类别",
  "mat.catNamePh": "类别名称",
  "mat.catDeleteQ": "确认删除该类别？其材料将移至：",
  "mat.catMoveTo": "请选择类别…",
  "mat.catDelete": "删除类别",
  "mat.cat.SAM": "湿法 SAM 材料",
  "mat.cat.PRECURSOR": "湿法钙钛矿前驱体",
  "mat.cat.EVAPORATION": "热蒸发材料",
  "mat.cat.SPUTTER": "溅射材料",
  "mat.cat.ALD": "原子层沉积（ALD）材料",
  "mat.cat.SOLVENT": "溶剂",
  "mat.cat.OTHER": "其他 / 配制溶液",
  "rec.title": "配方库",
  "rec.subtitle": "专有配方——查看权限与管理权限分开",
  "rec.add": "添加配方",
  "rec.new": "新配方",
  "rec.name": "配方名称",
  "rec.summary": "配方简介",
  "rec.summaryHint": "随配方内容授权可见，如「1.68 eV 宽带隙三阳离子」",
  "rec.components": "组分",
  "rec.component": "组分",
  "rec.amount": "用量",
  "rec.addComponent": "添加组分",
  "rec.solvents": "溶剂体系",
  "rec.concentration": "浓度",
  "rec.procedure": "配制步骤",
  "rec.lockedHint":
    "所有人均可创建和选用配方。创建者和负责人可查看内容；配方查看权限仅开放已批准内容。只有负责人可以编辑和审批。",
  "rec.hidden": "内容已隐藏——需要配方权限",
  "rec.empty": "暂无配方。",
  "users.perms": "附加权限",
  "users.permMaterials": "材料管理员",
  "users.permRecipes": "配方权限",
  "insp.searchMaterial": "输入以搜索材料…",
  "insp.createMaterial": "创建材料",
  "insp.materialSearchHint":
    "搜索支持模糊匹配，拼写略有出入也能找到。新材料由材料管理员在资源库中添加。",
  "insp.recipe": "配方",
  "insp.noRecipe": "无配方",
  "insp.recipeHidden": "已选择配方——内容仅配方权限成员可见。",
  "wf.assignee": "指派技术员",
  "wf.assigneeHint":
    "被指派者负责在实验室执行本实验并采集数据。移交实验室前必须指派。",
  "wf.unassigned": "尚未指派",
  "wf.startLab": "移交实验室",
  "wf.needAssignee": "请先指派负责人，再将实验移交实验室。",
  "wf.submit": "提交审核",
  "wf.submitTitle": "交回主管审核",
  "wf.submitHint": "主管将审核采集的数据并结束实验。",
  "wf.submitNote": "给主管的备注（可选）",
  "wf.submitted": "已提交审核",
  "wf.submittedBy": "提交于",
  "wf.awaiting": "等待您审批",
  "wf.reviewTitle": "主管审核",
  "wf.reviewHint":
    "批准后将封存证据包——方案、采集数据、照片与结果——并结束实验。",
  "wf.reviewNote": "结论备注 / 实验思考",
  "wf.approve": "批准并结束",
  "wf.approveQ": "确认批准并结束？",
  "wf.requestChanges": "退回实验室",
  "wf.changesQ": "确认按此备注退回？",
  "wf.approvedBy": "批准人",
  "wf.evidence": "证据包",
  "wf.evidenceHint": "批准时封存",
  "dash.newAs": "创建为",
  "dash.newReal": "正式实验",
  "dash.newTest": "测试",
  "dash.fromTpl": "从模板 / 以前的实验",
  "dash.tplSection": "置顶模板",
  "dash.recentSection": "我最近的实验",
  "dash.tplEmpty": "还没有可用的模板或最近的实验。",
  "dash.tplChip": "模板",
  "dash.pinTpl": "设为模板",
  "dash.unpinTpl": "取消模板",
  "trash.title": "回收箱",
  "trash.subtitle":
    "删除的实验会在这里保留约 30 天，之后自动清除。恢复后样品、数据、讨论全部找回。",
  "trash.empty": "回收箱是空的。",
  "trash.restore": "恢复",
  "trash.purge": "彻底删除",
  "trash.purgeQ": "确认彻底删除？",
  "trash.deletedBy": "删除者",
  "trash.purgeOn": "自动清除",
  "test.title": "测试数据",
  "test.subtitle": "以测试模式创建的实验——不会出现在任何正式视图中",
  "test.explain":
    "测试实验不会出现在实验看板、数据表、实验室端或个人统计中。它们用于试用系统，并可一次性全部清除。",
  "test.badge": "测试",
  "test.experiments": "个测试实验",
  "test.empty": "暂无测试实验。",
  "test.clear": "清除所有测试数据",
  "test.clearNow": "删除",
  "test.confirm":
    "确认永久删除 {n} 个测试实验及其全部样品、运行与结果？请输入 DELETE 确认。",
  "test.cleared": "已删除 {n} 个测试实验。",
  "test.promote": "标记为正式",
  "test.promoteHint": "将该实验移入正式数据库",
  "test.nav": "测试数据",
  "sum.dataPoints": "数据点",
  "sum.dataPointsHint": "实验室存储的每条测量曲线上的每一个 (x, y) 数据点——累计总数。",
  "sum.experiments": "实验",
  "sum.experimentsHint": "每个样品计为一个实验。",
  "sum.samples": "样品",
  "sum.results": "测量记录",
  "sum.runs": "运行",
  "sum.materials": "材料",
  "sum.recipes": "配方",
  "sum.files": "文件",
  "sum.search": "搜索",
  "sum.searching": "搜索中…",
  "sum.clear": "清除",
  "sum.searchPh": "搜索材料、工艺、配方、样品，或直接提问…",
  "sum.found": "找到 {n} 个相关实验",
  "sum.none": "没有匹配结果。",
  "ai.title": "AI 模型",
  "ai.subtitle": "密钥仅保存在服务器，保存后不再显示，只展示末尾几位",
  "ai.add": "添加模型",
  "ai.label": "名称",
  "ai.provider": "服务商",
  "ai.baseUrl": "接口地址",
  "ai.model": "模型",
  "ai.key": "API 密钥",
  "ai.keyKeep": "留空则保留已保存的密钥",
  "ai.active": "启用中",
  "ai.use": "启用",
  "ai.test": "测试",
  "ai.testing": "测试中…",
  "ai.save": "保存",
  "ai.cancel": "取消",
  "ai.remove": "删除",
  "ai.removeQ": "确认删除该模型？",
  "ai.empty": "尚未配置模型。在添加之前，自然语言搜索将退化为关键词搜索。",
  "ai.never": "未测试",
  "ai.nav": "AI 模型",
  "ai.loadModels": "获取模型",
  "ai.loading": "获取中…",
  "ai.liveModels": "该服务商当前提供 {n} 个模型",
  "ai.modelHint": "填入密钥后点击「获取模型」，查看该服务商当前可用的模型。",
  "exp.title": "数据导出",
  "exp.subtitleAdmin": "审批导出申请，并查看谁导出过数据",
  "exp.subtitleMine": "您的导出申请及状态",
  "exp.request": "申请导出",
  "exp.send": "提交申请",
  "exp.reasonPh": "请说明导出用途",
  "exp.requested": "已提交导出申请，等待管理员审批。",
  "exp.alreadyPending": "您已有一条待审批的申请。",
  "exp.denied": "该导出申请已被拒绝。",
  "exp.logged": "已导出，并记录到日志。",
  "exp.pending": "待审批",
  "exp.noPending": "暂无待审批申请。",
  "exp.history": "导出日志",
  "exp.noHistory": "暂无导出记录。",
  "exp.approve": "批准",
  "exp.deny": "拒绝",
  "exp.notePh": "备注（可选）",
  "exp.decidedBy": "审批人",
  "exp.downloaded": "下载于",
  "exp.status.PENDING": "待审批",
  "exp.status.APPROVED": "已批准",
  "exp.status.DENIED": "已拒绝",
  "exp.nav": "数据导出",
  "ing.title": "数据接入",
  "ing.subtitle": "审核并发布从文档与历史数据中提取的信息",
  "ing.howTitle": "使用方式",
  "ing.howBody":
    "把文件（配方表、材料说明书、设备手册、历史实验记录）交给智能体，它会提取信息并暂存在此——不会自动进入正式库。请逐条审核、修正后发布或拒绝。",
  "ing.pending": "待审核",
  "ing.reviewed": "已审核",
  "ing.emptyPending": "暂无待审核条目。",
  "ing.emptyDone": "暂无已审核条目。",
  "ing.review": "审核",
  "ing.publish": "批准并发布",
  "ing.reject": "拒绝",
  "ing.rejectQ": "确认拒绝该条目？",
  "ing.saveDraft": "保存修改",
  "ing.reviewNote": "审核备注",
  "ing.agentNote": "智能体说明",
  "ing.extras": "其他提取字段（不会发布）",
  "ing.published": "已发布",
  "ing.rejected": "已拒绝",
  "ing.by": "审核人",
  "ing.process": "工艺",
  "ing.pickProcess": "请选择工艺…",
  "ing.kind.MATERIAL": "材料",
  "ing.kind.EQUIPMENT": "设备",
  "ing.kind.EXPERIMENT": "实验",
  "ing.expTitle": "实验标题",
  "ing.operator": "实验人员",
  "ing.scale": "规模",
  "ing.batch": "批次",
  "ing.date": "日期",
  "ing.steps": "工艺步骤",
  "ing.characterizations": "表征",
  "ing.samples": "样品",
  "ing.withMetrics": "含数据",
  "ing.rawFiles": "原始文件",
  "ing.andMore": "…另有 {n} 个样品",
  "ing.kind.EXPERIMENT_": "",
  "ing.kind.FORMULA": "配方",
  "ing.kind.ENVIRONMENT": "实验环境",
  "ing.kind.PRESET": "工艺配方",
  "ing.presetName": "工艺配方名称",
  "ing.manage": "数据接入（暂存区）",
  "ing.composition": "化学组成",
  "ing.bandGap": "带隙 (eV)",
  "ing.role": "作用",
  "ing.formulaNotes": "原始文件备注",
  "ing.inLibrary": "库中已有",
  "ing.matches": "库中对应",
  "ing.notInLibrary": "库中没有",
  "ing.allKnown": "所有组分均已存在于材料库中。",
  "ing.unknownCount": "个组分不在材料库中——发布该配方不会自动创建它们。",
  "ing.dupFormula": "已存在同名配方——发布将直接覆盖更新该配方。",
  "ing.dupTitle": "库中已存在",
  "ing.dupBody":
    "该条目与库中已有记录重复。请选择：更新已有记录、跳过该条目，或确认另建一条新记录。",
  "ing.dupMatchedOn": "匹配依据：",
  "ing.dupIdentical": "完全相同——没有新增信息",
  "ing.dupUpdate": "更新这条",
  "ing.dupWillUpdate": "✓ 将更新",
  "ing.dupSkip": "作为重复项跳过",
  "ing.dupCreateNew": "另建一条记录",
  "ing.dupCreatingNew": "⚠ 将另建一条记录",
  "ing.dupBlocked": "请先处理重复项。",
  "ing.dupStatus": "重复",
  "ing.publishUpdate": "批准并更新",
  "ing.dupQueue": "队列中还有同名条目",
  "ing.selectAll": "全选",
  "ing.allKinds": "全部",
  "ing.selectAllKind": "全选{kind}",
  "ing.nSelected": "已选 {n} 项",
  "ing.bulkApprove": "批量批准",
  "ing.bulkRunning": "正在发布…",
  "ing.bulkConfirm": "确认发布 {n} 项到正式库？",
  "ing.bulkDone": "已发布 {total} 项中的 {n} 项。",
  "ing.dupWhatNext": "{n} 项因重复被保留——如何处理？",
  "ing.dupReplace": "覆盖已有记录",
  "ing.dupReplaceQ": "确认用暂存内容覆盖已有记录？",
  "ing.dupBulkSkip": "跳过（保留已有）",
  "ing.dupSkipQ": "确认标记为重复并关闭？",
  "ing.dupDelete": "从队列移除",
  "ing.dupDeleteQ": "确认删除这些暂存条目？",
  "ing.bulkHeldTitle": "以下条目未发布：",
  "ing.bulkHeldDup": "库中已存在：",
  "plan.equipment": "使用设备",
  "plan.anyEquipment": "不限 / 未指定",
  "plan.layer": "器件层",
  "plan.noLayer": "与层无关",
  "insp.layer": "器件层",
  "insp.noLayer": "与层无关",
  "insp.layerHint":
    "该步骤所构建的器件层——显示在流程卡片上，并用于归类实验方案。",
  "lib.defaultLayer": "默认器件层",
  "team.owner": "负责人（创建者）",
  "team.team": "团队",
  "team.firstIsResponsible": "——排在第一位者负责完成本实验",
  "team.lead": "负责",
  "team.searchFirst": "搜索人员…",
  "team.searchMore": "添加参与人…",
  "team.dragHint": "拖动可调整顺序——第一位为负责人",
  "team.remove": "移除",
  "team.noMatch": "未找到匹配人员。",
  "org.title": "机构管理",
  "org.subtitle": "机构设置、成员、角色与各领域负责人",
  "org.settings": "机构设置",
  "org.name": "机构名称",
  "org.number": "机构编号",
  "org.numberHint": "用于所有实验编号——不可更改",
  "org.responsible": "负责人",
  "org.responsibleHint":
    "指定各部分资源库的维护人。同一职责可由多人共同承担；管理员始终拥有全部职责。",
  "org.stewMaterials": "材料",
  "org.stewMaterialsHint": "添加与编辑材料、类别及供应商/批次信息",
  "org.stewEquipment": "设备",
  "org.stewEquipmentHint": "添加与编辑设备及其参数集",
  "org.stewFacilities": "场地设施",
  "org.stewFacilitiesHint": "管理位置与实验室环境",
  "org.stewRecipes": "配方",
  "org.stewRecipesHint": "查看已批准配方内容，不包含编辑权限",
  "org.recipeSteward": "配方负责人",
  "org.stewDevices": "共享平板",
  "org.stewDevicesHint": "注册共享平板、发放设置链接、吊销设备",
  "org.recipeStewardHint": "查看、编辑、归档和审批配方",
  "review.materials": "材料修改建议",
  "review.submit": "提交审批",
  "review.pending": "待审批",
  "review.approved": "已批准",
  "review.rejected": "已拒绝",
  "review.approve": "批准",
  "review.reject": "拒绝",
  "review.failed":
    "操作未完成。请刷新并检查权限；若材料已变更，请重新提交修改建议。",
  "review.savedOnly": "审批针对已保存的内容。请先保存修改。",
  "org.noneAssigned": "尚未指定",
  "org.adminImplicit": "管理员自动拥有全部职责。",
  "org.you": "本人",
  "org.manage": "机构管理（管理员）",
  "orgs.title": "平台机构",
  "orgs.subtitle": "平台上的可信机构——审批入驻申请并管理邮箱域名",
  "orgs.manage": "机构管理（平台管理员）",
  "orgs.name": "机构",
  "orgs.admin": "指定管理员",
  "orgs.domains": "邮箱域名",
  "orgs.members": "成员数",
  "orgs.status": "状态",
  "orgs.active": "已信任",
  "orgs.pending": "待审批",
  "orgs.approve": "批准",
  "orgs.reject": "拒绝",
  "orgs.rejectQ": "确认拒绝并移除？",
  "orgs.editDomains": "点击编辑域名",
  "orgs.invite": "邀请机构入驻",
  "orgs.inviteHint":
    "生成一次性链接并发送给对方机构代表。对方登记机构信息及其管理员账号后，将显示在上方列表中等待您批准。",
  "orgs.generateLink": "生成邀请链接",
  "orgs.copy": "复制",
  "orgs.copied": "已复制 ✓",
  "orgs.linkTtl": "链接 7 天内有效，仅可使用一次。",
  "onboard.title": "登记您的机构",
  "onboard.subtitle": "您受邀以可信机构身份加入 Pheno 实验数据平台。",
  "onboard.orgName": "机构名称",
  "onboard.domains": "邮箱域名",
  "onboard.domainsHint": "逗号分隔。仅这些域名的邮箱可注册到您的机构。",
  "onboard.adminSection": "指定管理员账号",
  "onboard.adminEmail": "管理员邮箱（须属于上方域名之一）",
  "onboard.submit": "提交审批",
  "onboard.done": "已提交。您的机构正在等待审批——平台管理员批准后即可登录。",
  "onboard.badToken":
    "邀请链接无效、已过期或已被使用。请向平台管理员索取新链接。",
  "onboard.emailDomain": "管理员邮箱必须属于所列域名之一。",
  "onboard.badInput": "请检查机构名称、域名与邮箱，密码至少 8 位。",
  "cap.confirmFor": "确认",
  "cap.capturedSamples": "已采集",
  "cap.simCode": "样品代码",
  "cap.applyToAll": "应用到全部",
  "cap.trashWhy": "这片基片出了什么问题？（记录到样品；照片可在工序卡上添加）",
  "cap.scanPolicy": "显示数值（多次扫描）：",
  "cap.policy.BEST": "最佳",
  "cap.policy.MIN": "最低",
  "cap.policy.AVERAGE": "平均",
  "cap.policy.MEDIAN": "中位",
  "cap.perSampleHint": "表征结果逐个样品录入。",
  "cap.overwriteQ": "以下样品已有采集记录，将被覆盖：",
  "cap.overwrite": "覆盖",
  "cap.addPhoto": "添加照片",
  "cap.photos": "照片",
  "cap.noPhotos": "暂无照片——可从相机或相册添加。",
  "cap.photoOf": "照片",
  "cap.deletePhotoQ": "删除照片？",
  "cap.confirmDeletePhoto": "确认删除照片",

  "res.title": "结果",
  "res.compare": "结果对比",
  "res.subtitle": "各变量组的测量结果并排对比",
  "res.group": "组别",
  "res.mean": "平均",
  "res.noResults": "暂无测量结果——请在采集页面录入。",
  "res.variables": "被测变量",
  "res.samples": "样品",
  "di.title": "AI 数据分析 — 基于全部实验历史提问",
  "di.hint":
    "回答基于你可见的全部实验（不含测试实验），并引用实验编号。约需 30–60 秒。",
  "di.ph": "例如：旋涂SAM和刮涂SAM哪个更好？与哪种HTL搭配更优？",
  "di.run": "分析",
  "di.running": "分析中…",
  "di.error": "分析失败或未配置 AI 服务，请重试。",
  "di.footer": "基于 {n} 个实验 · {model} · AI 生成，请对照记录核实",
  "res.dist": "分组分布图",
  "res.distHint": "箱体 = 中间50% · 横线 = 中位数 · △ = 均值 · 圆点 = 各样品",
  "res.open": "查看结果",
  "res.run": "批次",
  "rep.title": "报告",
  "rep.subtitle": "实验报告",
  "rep.print": "打印 / PDF",
  "labels.title": "样品标签",
  "labels.hint":
    "A4 不干胶标签纸，每页 21 枚（70 × 42.3 mm）。请按 100% 缩放、无边距打印。",
  "labels.empty": "该实验还没有样品。",
  "scan.button": "扫标签",
  "scan.hint": "将相机对准样品标签二维码",
  "scan.fail": "无法打开相机——请检查相机权限后重试。",
  "scan.cancel": "取消",
  "rep.method": "科学方法",
  "rep.plan": "实验方案",
  "rep.flow": "工艺流程",
  "rep.charPlan": "表征",
  "rep.results": "结果",
  "rep.team": "团队",
  "rep.created": "创建时间",
  "rep.completed": "状态",
  "rep.materials": "材料",
  "rep.equipment": "设备",
  "rep.environment": "环境",
  "rep.parameters": "参数",
  "rep.deviations": "采集偏差与备注",
  "rep.noDeviations": "未记录与方案的偏差。",
  "rep.generated": "由 Pheno 实验数据平台生成",

  "reg.title": "创建账户",
  "reg.subtitle": "注册使用发送到机构邮箱的一次性验证码。",
  "reg.email": "机构邮箱",
  "reg.request": "发送验证码",
  "reg.codeSent":
    "一次性验证码已生成。请在下方输入——如未收到，请联系管理员（管理员可在「用户 → 待注册」中查看验证码）。",
  "reg.codeEmailed":
    "一次性验证码已发送至您的邮箱。请在下方输入——若一分钟内未收到，请检查垃圾邮件文件夹。",
  "reg.code": "一次性验证码",
  "reg.name": "您的姓名",
  "reg.password": "设置密码",
  "reg.password2": "确认密码",
  "reg.submit": "验证并创建账户",
  "reg.done": "注册已提交——管理员审批通过后即可登录。",
  "reg.badDomain": "该邮箱域名未注册任何机构。请联系管理员添加。",
  "reg.badCode": "验证码无效或已过期。",
  "reg.exists": "该邮箱已注册账户。",
  "reg.link": "新用户？使用机构邮箱注册",
  "reg.backToLogin": "返回登录",
  "reset.title": "重置密码",
  "reset.subtitle": "输入账号邮箱，我们将发送一次性验证码。",
  "reset.request": "发送重置验证码",
  "reset.codeEmailed":
    "如果该邮箱有账号，验证码已发送。请在下方输入——未收到请检查垃圾邮件。",
  "reset.codeSent":
    "如果该邮箱有账号，验证码已生成。请向管理员索取（管理员可在“成员 → 待处理验证码”中查看）。",
  "reset.password": "新密码",
  "reset.submit": "设置新密码",
  "reset.done": "密码已更新，现在可以用它登录。",
  "login.forgot": "忘记密码？",
  "users.resetPw": "重置密码",
  "users.resetConfirm": "使用此密码",
  "users.resetDone": "密码已设置——请立即复制，之后不再显示。",
  "users.codeRegister": "注册",
  "users.codeReset": "重置",

  "users.title": "用户",
  "users.subtitle": "管理机构的账户与权限",
  "users.name": "姓名",
  "users.email": "邮箱",
  "users.role": "角色",
  "users.team": "项目组",
  "users.ungrouped": "— 未分组 —",
  "users.active": "启用",
  "users.joined": "加入时间",
  "users.deactivate": "停用",
  "users.activate": "启用",
  "users.domains": "允许注册的邮箱域名",
  "users.domainsHint":
    "仅这些域名的邮箱可注册到本机构。逗号分隔，如 ultiprice.com, pheno.lab",
  "users.saveDomains": "保存域名",
  "users.pending": "待注册",
  "users.pendingHint":
    "有效的一次性验证码——如未配置邮件发送，请将验证码告知注册者。",
  "users.noPending": "暂无待注册。",
  "users.expires": "过期时间",
  "users.manage": "用户管理（管理员）",

  "sys.title": "系统与存储",
  "sys.subtitle": "数据存放位置、占用空间与备份",
  "sys.database": "数据库",
  "sys.dbLocation": "位置",
  "sys.dbName": "数据库名",
  "sys.dbSize": "数据库大小",
  "sys.uploads": "上传文件（照片与截图）",
  "sys.disk": "磁盘",
  "sys.diskFree": "可用 / 共",
  "sys.tables": "最大数据表",
  "sys.backups": "备份",
  "sys.backupNow": "立即备份",
  "sys.backupDone": "备份已创建。",
  "sys.noBackups": "暂无备份。",
  "sys.backupHint":
    "备份为 pg_dump 快照，保存在 backups/ 目录并自动轮换（保留最近 30 份）。本机已安装每日定时备份。",
  "sys.backupExternalHint":
    "数据库备份和恢复由外部 PostgreSQL 服务器统一管理。",
  "sys.migrateHint":
    "将来迁移存储（NAS/云）时，只需将 DATABASE_URL 指向新的 PostgreSQL 主机并恢复最新备份——应用无需改动。",
  "sys.manage": "系统与存储（管理员）",

  // 设备数据（J-V 自动接入）
  "inst.title": "设备数据",
  "inst.subtitle": "太阳光模拟器自动上传的测试数据",
  "inst.nav": "测试设备",
  "inst.manage": "设备数据与采集程序",
  "inst.rigs": "已连接设备",
  "inst.none": "尚未注册任何测试设备。",
  "inst.lastSeen": "最近上报",
  "inst.never": "从未",
  "inst.online": "在线",
  "inst.offline": "未上报",
  "inst.host": "电脑",
  "inst.watching": "监控目录",
  "inst.inbox": "接收到的测试数据",
  "inst.ownedBy": "指派给",
  "inst.selectAll": "全选",
  "inst.nSelected": "已选 {n} 条",
  "inst.handTo": "转交给…",
  "inst.handedOver": "已将 {n} 条测量转交给 {name}",
  "inst.matched": "已匹配",
  "inst.unmatched": "待处理",
  "inst.ignored": "已忽略",
  "inst.serial": "设备中填写的编号",
  "inst.sample": "样品",
  "inst.measuredAt": "测试时间",
  "inst.scan": "扫描方向",
  "inst.instrument": "设备",
  "inst.emptyInbox": "暂无数据。",
  "inst.emptyUnmatched": "所有数据均已匹配。",
  "inst.rematch": "重新匹配",
  "inst.rematching": "匹配中…",
  "inst.rematchDone": "{considered} 条中已匹配 {matched} 条。",
  "inst.assign": "归入…",
  "inst.detach": "取消归属",
  "inst.ignore": "忽略",
  "inst.pickSample": "选择样品",
  "inst.autoHint":
    "数据会自动上传——每台模拟器电脑上的采集程序会推送新测试文件。未匹配的数据每隔几分钟自动重试，因为测试数据常常先于实验记录到达。",

  "jv.title": "J-V 数据文件",
  "jv.pull": "拉取 J-V 文件",
  "jv.pulling": "拉取中…",
  "jv.pulled": "新匹配 {matched} 条 · 已关联 {files} 个文件",
  "jv.noneYet": "该实验尚未匹配到 J-V 文件。",
  "jv.samples": "样品",
  "jv.scansOne": "1 次扫描",
  "jv.scansMany": "{n} 次扫描",
  "jv.noScans": "暂无文件",
  "jv.best": "最佳",
  "jv.candidates": "近 30 天未匹配的文件",
  "jv.candidatesHint": "若操作员填错编号，可在此手动归入正确样品。",
  "jv.attach": "归入",
  "jv.attached": "已关联文件",
  "jv.hint":
    "操作员需在设备中填写样品编号（例如 {example}），匹配成功的数据会自动出现在这里。",
  "jv.serials": "设备中填写的编号",
  "jv.serialsHint":
    "在设备的 Serial NO. / Sample name 栏填写该编号；像素号写在后面，例如 {example}-2。",
  "jv.copyAll": "复制全部",
  "jv.copied": "已复制",
  "jv.alias": "同时匹配",
  "jv.aliasAdd": "添加其他编号",
  "jv.aliasSaved": "编号已添加",
  "kiosk.title": "共享平板",
  "kiosk.subtitle":
    "注册过的实验室平板以共享模式运行：点名快捷登录、闲置 1 小时自动退出、同时仅一人登录、不显示管理页面。",
  "kiosk.labelPh": "例如：3号平板 · A2-202 蒸镀区",
  "kiosk.nameField": "为这台平板命名",
  "kiosk.nameHint":
    "填写能说明是哪台平板、放在哪个区域的名字——管理员列表中显示的就是这个名字。",
  "kiosk.pendingName": "（待注册——等待技术员在设备上命名）",
  "kiosk.create": "生成注册链接",
  "kiosk.addHint":
    "链接已自动复制——发给技术员，由其在平板上打开、命名设备并确认。",
  "kiosk.none": "尚未注册共享平板。",
  "kiosk.unclaimed": "设置链接尚未在设备上打开",
  "kiosk.inUse": "{name} 已登录 · {ago}前有操作",
  "kiosk.idle": "当前无人登录 · 最近操作 {ago}前",
  "kiosk.revoked": "已停用——该设备上的会话已被阻止",
  "kiosk.copyLink": "复制设置链接",
  "kiosk.copied": "已复制",
  "kiosk.revoke": "停用",
  "kiosk.restore": "恢复",
  "kiosk.rules":
    "共享会话闲置 1 小时自动退出，单次登录最长 12 小时；新登录会自动顶掉忘记退出的上一个人。",
  "kiosk.manage": "共享平板",
  "kiosk.claimTitle": "将此浏览器注册为共享平板",
  "kiosk.claimHint":
    "请仅在实验室平板上确认，不要在个人手机上操作。注册后该浏览器会自动退出闲置用户。",
  "kiosk.claimConfirm": "确认——这是实验室平板",
  "kiosk.claimInvalid": "设置链接无效或已被使用。",
  "kiosk.claimDone": "平板注册成功，请在下方登录。",
  "kiosk.loginHint":
    "共享平板：点击你的名字，输入密码登录。闲置 1 小时将自动退出。",
  "kiosk.badgeHint": "共享平板会话——闲置时会自动退出登录。",
  "kiosk.idleTitle": "还在使用吗？",
  "kiosk.idleBody": "长时间无操作，{s} 秒后将自动退出以保护你的账号。",
  "kiosk.idleStay": "继续使用",
  "nfc.title": "NFC 工牌",
  "nfc.hint":
    "绑定一次工牌，以后在任何共享平板上贴卡即可登录，无需输入。绑定需在安卓 Chrome 上进行（建议在共享平板上正常登录后操作）。",
  "nfc.bound": "工牌已绑定到此账号",
  "nfc.bind": "绑定我的工牌",
  "nfc.rebind": "绑定新工牌",
  "nfc.unbind": "解绑",
  "nfc.unbound": "工牌已解绑。",
  "nfc.tapNow": "请将工牌贴在设备背面…",
  "nfc.holdStill": "保持工牌贴住不动，正在写入…",
  "nfc.bindDone": "绑定成功。以后在共享平板上贴卡即可登录。",
  "nfc.bindDoneUidOnly": "绑定成功（芯片写保护，仅按序列号绑定）。",
  "nfc.bindFailed": "绑定失败，请重试。",
  "nfc.timeout": "未检测到工牌——请重试并保持卡片贴稳。",
  "nfc.unsupported":
    "当前浏览器不支持 NFC 读取。请在安卓平板或手机的 Chrome 上绑定。",
  "nfc.loginBtn": "贴工牌登录",
  "nfc.loginTap": "请将工牌贴在平板背面…",
  "nfc.loginChecking": "正在验证工牌…",
  "nfc.loginFailed": "未识别的工牌。请先在个人资料页绑定，或使用密码登录。",
  "nfc.loginRetry": "重试",
  "users.editIdentity": "编辑姓名和邮箱",
  "users.cancel": "取消",
  "login.switchUser": "不是你？",
  "login.pending": "你的账号正在等待管理员审批，通过后即可登录。",
  "appr.title": "待审批的注册申请",
  "appr.hint":
    "新注册的账号需你审批后才能使用。可在此统一姓名/Handle/邮箱格式；若此人有导入的历史数据，也在此一并认领。",
  "appr.registered": "注册信息",
  "appr.handle": "Handle",
  "appr.legacy": "历史数据",
  "appr.legacyHint":
    "如果此人平台上线前的实验已被导入，选择对应数据集——批准的同时数据即归其所有。",
  "appr.noLegacy": "无历史数据",
  "appr.expCount": "{n} 个实验",
  "appr.suggested": "姓名匹配",
  "appr.approve": "批准",
  "appr.reject": "拒绝",
  "req.ownedBy": "{name} 的实验",
  "req.explain":
    "这是同事的实验。提交访问申请，实验负责人（或管理者）批准后即可打开，并自动成为实验成员。",
  "req.declined": "上次申请被拒绝，可以再次申请。",
  "req.messagePh": "说明申请原因（选填）",
  "req.send": "申请访问",
  "req.sending": "提交中…",
  "req.pending": "申请已发送——等待 {name} 批准。",
  "req.asksAccess": "申请访问此实验",
  "req.approve": "批准",
  "req.decline": "拒绝",
  "notif.draft_idle_warning": "将在 3 天后归档你闲置的草稿——打开并编辑即可保留",
  "notif.draft_idle_archived": "已归档你闲置的草稿——打开后一键即可恢复",
  "idle.warnChip": "闲置 · 3 天后归档",
  "idle.warnHint": "7 天没有编辑。任何编辑都会保留它；否则将自动归档。",
  "idle.archivedChip": "因闲置已归档",
  "idle.archivedHint": "闲置 10 天后自动归档。点击恢复即可继续。",
  "idle.renew": "恢复草稿",
  "idle.archivedBanner": "此草稿因长期未编辑已自动归档。内容没有删除。",
  "idle.warnBanner":
    "已闲置 7 天——如 3 天内没有任何编辑，将自动归档。任何修改都算。",
  "notif.title": "通知",
  "notif.loading": "加载中…",
  "notif.empty": "暂无通知——同事间的互动会显示在这里。",
  "notif.justNow": "刚刚",
  "notif.access_requested": "申请访问你的实验",
  "notif.access_approved": "批准了你的访问申请",
  "notif.access_declined": "拒绝了你的访问申请",
  "notif.feedback_approved": "批准了你的反馈",
  "notif.feedback_rejected": "拒绝了你的反馈",
  "notif.feedback_implemented": "已实现你的反馈——请到个人页测试并确认",
  "notif.feedback_commented": "评论了你的反馈",
  "notif.feedback_verified": "确认了你的实现有效",
  "notif.feedback_reopened": "退回了一条已实现的反馈",
  "notif.assigned": "给你分配了一个实验",
  "notif.member_added": "将你加入了一个实验",
  "notif.material_edit_requested": "提交了材料修改建议",
  "notif.material_edit_approved": "批准了你的材料修改建议",
  "notif.material_edit_rejected": "拒绝了你的材料修改建议",
  "notif.recipe_approval_requested": "提交了待审批配方",
  "notif.recipe_approved": "批准了你的配方",
  "notif.recipe_rejected": "拒绝了你的配方",
  "notif.experiment_commented": "评论了你的实验",
  "notif.mentioned": "在评论中提到了你",
  "cmt.title": "讨论",
  "cmt.empty": "还没有评论——输入 @ 可以召唤同事。",
  "cmt.placeholder": "写评论… @ 提及会通知对方",
  "cmt.post": "发送",
  "cmt.delete": "删除评论",
  "jvr.unlinked": "{n} 条仪器扫描未链接到样品",
  "jvr.hint":
    "在仪器上输错样品编号会导致扫描无法自动链接。为每条扫描选择所属样品并链接，数值会立即填入。",
  "jvr.pick": "选择样品…",
  "jvr.suggested": "建议",
  "jvr.link": "链接",
  "jvr.linked": "已链接，数值已填入。",
  "jvr.others": "疑似属于其他实验的扫描",
};

export type Lang = "en" | "zh";
export const dictionaries: Record<Lang, Record<TKey, string>> = { en, zh };

export function translate(
  lang: Lang,
  key: TKey,
  vars?: Record<string, string>,
): string {
  let s: string = dictionaries[lang][key] ?? en[key] ?? key;
  if (vars)
    for (const [k, v] of Object.entries(vars)) s = s.replaceAll(`{${k}}`, v);
  return s;
}
