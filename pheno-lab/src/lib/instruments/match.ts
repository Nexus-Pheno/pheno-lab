 ...
 */
async function matchByInstrumentCode(organizationId: string, serial: string, key: string): Promise<MatchResult> {
  ...
  const samples = await prisma.sample.findMany({
    where: { instrumentCodes: { hasSome: candidates }, experiment: { organizationId } },
    select: {
      id: true,
      code: true,
      instrumentCodes: true,
      experimentId: true,
      experiment: { select: { code: true, runs: { select: { id: true }, orderBy: { runNo: "desc" }, take: 1 } } },
    },
  });
  if (!samples.length) {
    return unmatched(
  ...
  for (const candidate of candidates) {
    ...
    if (hits.length > 1) {
      const where = hits.map((h) => `${h.experiment.code}-${h.code}`).join(", ");
      return unmatched(`"${serial}" is claimed by more than one sample (${where}); remove the duplicate serial.`);
    }
    const hit = hits[0];
    ...
    return {
      ...
      matchNote:
        `Matched on serial "${candidate}"` + (pixel ? ` · pixel ${pixel} — recorded at sample level.` : "."),
    };
  }
  return unmatched(`"${serial}" does not match any sample's instrument serial.`);
}
 ...
 */
export async function matchSerial(organizationId: string, serial: string): Promise<MatchResult> {
  ...
  const experiment = await prisma.experiment.findFirst({
    ...
    select: {
      ...
      runs: { select: { id: true, runNo: true }, orderBy: { runNo: "desc" }, take: 1 },
    },
  });
  if (!experiment) return unmatched(`No experiment ${code} in this organization.`);
  ...
  if (!rest) {
    ...
    return unmatched(`"${serial}" names experiment ${code} but no sample (e.g. -S1).`);
  }

  const [sampleCode, ...pixelParts] = rest.split("-");
  const sample = experiment.samples.find((s) => s.code.toUpperCase() === sampleCode);
  if (!sample) {
    const known = experiment.samples.map((s) => s.code).join(", ") || "none";
    return unmatched(`Experiment ${code} has no sample "${sampleCode}" (has: ${known}).`);
  }
  ...
  return {
    ...
    sampleId: sample.id,
    runId,
    matchNote: pixelParts.length ? `Pixel ${pixelParts.join("-")} — recorded at sample level.` : "",
  };
}
...
const DECIMALS: Record<keyof typeof METRIC_LABELS, number> = { pce: 2, voc: 4, jsc: 2, ff: 2 };
...
export function metricsToResult(m: JvMetrics): Record<string, string> {
  ...
  for (const key of Object.keys(METRIC_LABELS) as (keyof typeof METRIC_LABELS)[]) {
    const v = m[key];
    if (typeof v === "number" && Number.isFinite(v)) out[METRIC_LABELS[key]] = v.toFixed(DECIMALS[key]);
  }
  return out;
}

/** The J-V characterization card of an experiment, if the designer has one. */
async function findJvCharacterization(experimentId: string): Promise<string | null> {
  const cards = await prisma.characterization.findMany({
    where: { experimentId },
    select: { id: true, name: true, position: true, process: { select: { name: true } } },
    orderBy: { position: "asc" },
  });
  const isJv = (s: string) => /j-?v|i-?v|current[-\s]?voltage|solar|效率|光电/i.test(s);
 ...
 */
export async function refreshSampleJvResult(sampleId: string): Promise<"written" | "kept-manual" | "no-card" | "no-data"> {
  ...
  if (existing && existing.source === "MANUAL" && Object.keys(existing.metrics ?? {}).length > 0) {
    return "kept-manual";
  }
  ...
  if (existing) {
    await prisma.characterizationResult.update({ where: { id: existing.id }, data });
  } else {
