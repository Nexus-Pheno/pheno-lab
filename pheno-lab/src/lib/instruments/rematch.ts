...
import { matchSerial, refreshSampleJvResult } from "./match";
...
export async function rematchMeasurements(opts: {
  ...
  const prefixes = (serialPrefixes ?? []).filter(Boolean).map((p) => p.toUpperCase());
  ...
  await db.jvMeasurement.updateMany({
  ...
  const pending = await db.jvMeasurement.findMany({
    where: {
      organizationId,
      status: "UNMATCHED",
      ...(prefixes.length ? { OR: prefixes.map((p) => ({ serialKey: { startsWith: p } })) } : {}),
    },
  ...
  for (const m of pending) {
    ...
    if (result.status !== "MATCHED") {
      ...
      await db.jvMeasurement.update({ where: { id: m.id }, data: { matchNote: result.matchNote } });
      continue;
    }
...
export async function rematchAllOrganizations(): Promise<Record<string, RematchSummary>> {
  const orgs = await db.organization.findMany({ where: { status: "ACTIVE" }, select: { id: true, slug: true } });
