...
import { chat, jsonFrom } from "@/lib/ai/chat";
 ...
 */
async function countDataPoints(orgId: string, includeTest: boolean): Promise<number> {
  const expWhere = includeTest ? { organizationId: orgId } : { organizationId: orgId, isTest: false };

  const [params, results, execs, materials] = await Promise.all([
    db.stepParameter.count({ where: { step: { experiment: expWhere } } }),
    db.characterizationResult.findMany({
      where: { characterization: { experiment: expWhere } },
      select: { metrics: true },
    }),
    db.stepExecution.findMany({
      where: { step: { experiment: expWhere } },
  ...
  for (const e of execs) n += jsonKeys(e.actuals) + jsonKeys(e.environmentConditions);
  for (const m of materials) n += jsonKeys(m.properties);
  return n;
}

export async function getDatabaseSummary(includeTest = false): Promise<DatabaseSummary> {
  const session = await requireSession();
  const org = session.org;
  const where = await canViewWhere(session, includeTest);
  const expWhere = includeTest ? { organizationId: org } : { organizationId: org, isTest: false };

  const [
    experiments, samples, runs, steps, characterizations, results, attachments,
    materials, recipes, equipment, processes, testExperiments, dataPoints,
  ] = await Promise.all([
    db.experiment.count({ where }),
    db.sample.count({ where: { experiment: expWhere } }),
    db.run.count({ where: { experiment: expWhere } }),
    db.processStep.count({ where: { experiment: expWhere } }),
    db.characterization.count({ where: { experiment: expWhere } }),
    db.characterizationResult.count({ where: { characterization: { experiment: expWhere } } }),
    db.attachment.count({
      where: {
        OR: [
          { characterizationResult: { characterization: { experiment: expWhere } } },
          { stepExecution: { step: { experiment: expWhere } } },
        ],
      },
    }),
    ...
    db.experiment.count({ where: { organizationId: org, isTest: true } }),
    countDataPoints(org, includeTest),
  ]);

  return {
    experiments, samples, runs, steps, characterizations, results, attachments,
    materials, recipes, equipment, processes, dataPoints, testExperiments,
  };
}
...
const STOP = new Set([
  "the", "a", "an", "of", "in", "on", "for", "with", "and", "or", "to", "is", "are",
  "what", "which", "show", "me", "all", "any", "find", "experiments", "experiment",
  "that", "used", "use", "using", "was", "were", "did", "do", "have", "has", "how",
  "best", "highest", "most", "was", "从", "的", "了", "和", "与", "哪些", "实验",
]);
 ...
 */
export async function searchExperiments(query: string, includeTest = false): Promise<SearchResponse> {
  const session = await requireSession();
  const org = session.org;
  const where = await canViewWhere(session, includeTest);
  const q = (query ?? "").trim();
  ...
  if (/\s/.test(q.trim()) && q.trim().split(/\s+/).length >= 4) {
    const [mats, procs, recs] = await Promise.all([
      db.material.findMany({ where: { organizationId: org, archived: false }, select: { name: true }, take: 400 }),
      db.process.findMany({ where: { organizationId: org, archived: false }, select: { name: true } }),
      db.recipe.findMany({ where: { organizationId: org, archived: false }, select: { name: true } }),
    ]);
    const reply = await chat(org, [
      {
        role: "system",
        content:
      ...
      },
      {
        role: "user",
        content:
          `Question: ${q}\n\nMaterials: ${mats.map((m) => m.name).slice(0, 400).join(", ")}` +
          `\n\nProcesses: ${procs.map((p) => p.name).join(", ")}` +
          `\n\nFormulas: ${recs.map((r) => r.name).join(", ")}`,
      },
    ], { maxTokens: 200 });
    const parsed = jsonFrom(reply);
    const picked = Array.isArray(parsed?.terms) ? (parsed!.terms as unknown[]) : [];
    const clean = picked.map((x) => String(x).trim()).filter((x) => x.length > 1).slice(0, 6);
    if (clean.length) {
      ts = clean;
  ...
  const contains = ts.map((t) => ({ contains: t, mode: "insensitive" as const }));
  ...
  const textOr = ts.flatMap((t) => [
    ...
    { samples: { some: { code: { contains: t, mode: "insensitive" as const } } } },
  ]);

  const or: Record<string, unknown>[] = [...textOr];
  if (materials.length)
    or.push({ steps: { some: { materials: { some: { materialId: { in: materials.map((m) => m.id) } } } } } });
  if (processes.length) {
    or.push({ steps: { some: { processId: { in: processes.map((p) => p.id) } } } });
    or.push({ characterizations: { some: { processId: { in: processes.map((p) => p.id) } } } });
  }
  if (recipes.length) or.push({ steps: { some: { recipeId: { in: recipes.map((r) => r.id) } } } });
  ...
  const rows = await db.experiment.findMany({
    ...
    take: 60,
    select: {
      id: true, code: true, title: true, status: true, isTest: true,
      campaign: true, hypothesis: true, conclusion: true,
  ...
  const hits: SearchHit[] = rows.map((r) => {
    ...
    for (const s of r.steps) {
      for (const m of s.materials) {
        if (matIds.has(nameKey(m.material.name)) && !seen.has("m" + m.material.name)) {
          seen.add("m" + m.material.name);
          reasons.push(`material: ${m.material.name}`);
        }
      }
      if (s.process && procIds.has(nameKey(s.process.name)) && !seen.has("p" + s.process.name)) {
        seen.add("p" + s.process.name);
        reasons.push(`process: ${s.process.name}`);
      }
      if (s.recipe && recIds.has(nameKey(s.recipe.name)) && !seen.has("r" + s.recipe.name)) {
        seen.add("r" + s.recipe.name);
        reasons.push(`formula: ${s.recipe.name}`);
      }
    }
    const hay = `${r.title} ${r.code} ${r.campaign} ${r.hypothesis} ${r.conclusion}`.toLowerCase();
    for (const t of ts) if (hay.includes(t.toLowerCase())) reasons.push(`text: “${t}”`);
    return {
      id: r.id, code: r.code, title: r.title, status: r.status, isTest: r.isTest,
