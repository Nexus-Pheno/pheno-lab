...
function OnboardForm() {
  ...
  useEffect(() => {
    ...
    checkInvite(token).then((r) => setTokenState(r.valid ? "valid" : "invalid"));
  }, [token]);

  const errText = (e: string) =>
    e === "exists" ? t("reg.exists")
      : e === "email-domain" ? t("onboard.emailDomain")
        : e === "bad-token" ? t("onboard.badToken")
          : t("onboard.badInput");
  ...
  return (
    <main className="min-h-dvh bg-subtle flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-surface border border-line rounded-[8px] p-8">
        ...
        <h1 className="text-sm font-semibold text-charcoal mb-1">{t("onboard.title")}</h1>
        <p className="text-[11.5px] text-muted mb-5">{t("onboard.subtitle")}</p>

        {tokenState === "checking" && <p className="text-sm text-muted">…</p>}
        ...
        {tokenState === "valid" && !done && (
          <div className="space-y-3.5">
            <div>
              ...
              <input className={inputCls} value={orgName} autoFocus onChange={(e) => setOrgName(e.target.value)} />
            </div>
            <div>
              <FieldLabel>{t("onboard.domains")}</FieldLabel>
              <input className={inputCls + " mono"} placeholder="acme.com, acme.cn" value={domains} onChange={(e) => setDomains(e.target.value)} />
              <p className="text-[10px] text-muted mt-1">{t("onboard.domainsHint")}</p>
            </div>
            <div className="border-t border-line pt-3.5">
              <p className="text-[11px] font-bold uppercase text-muted mb-2">{t("onboard.adminSection")}</p>
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input type="email" className={inputCls + " mono"} value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} />
                </div>
            ...
            <button
              disabled={busy || !orgName.trim() || !domains.trim() || !adminEmail.includes("@") || pw.length < 8}
              onClick={async () => {
                if (pw !== pw2) { setError(t("profile.passwordMismatch")); return; }
                setBusy(true);
                setError("");
                const res = await submitOrganization({
                  token, orgName, domainsCsv: domains, adminName, adminEmail, password: pw,
                });
                setBusy(false);
        ...
        {done && (
          <div className="space-y-4">
            ...
            <Link href="/login" className="block text-center text-[12.5px] font-semibold text-charcoal hover:underline">
              {t("reg.backToLogin")}
            </Link>
          </div>
        )}
