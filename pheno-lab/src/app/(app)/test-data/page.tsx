...
import { listTestExperiments } from "@/lib/actions/experiments";
...
export default async function TestDataPage() {
  ...
  const t = await getT();
  const rows = await listTestExperiments();
      ...
      <div className="max-w-4xl mx-auto p-3 sm:p-6 space-y-5">
        <div>
          <h1 className="text-lg font-bold flex items-center gap-2">
            <Icon name="FlaskConical" size={17} className="text-warn" /> {t("test.title")}
          </h1>
