...
// Opened ON the tablet from the setup link. Confirming pins the shared-device
// cookie into this browser — a button, not an automatic GET side effect, so a
// link preview or prefetch cannot consume the one-time token.
  ...
  return (
    <main className="min-h-dvh bg-subtle flex items-center justify-center p-6">
      <div className="w-full max-w-sm bg-surface border border-line rounded-[8px] p-8 text-center">
        ...
        {device ? (
          <>
            <h1 className="text-[15px] font-bold mb-1">{t("kiosk.claimTitle")}</h1>
            <p className="text-[13px] text-charcoal mb-1">
              <span className="font-bold text-brand-deep">{device.label}</span>
            </p>
            <p className="text-[12px] text-muted mb-5">{t("kiosk.claimHint")}</p>
            <form
              action={async () => {
                "use server";
