...
- `src/infrastructure/push/**`：可选 DingTalk 群机器人 adapter；通知模块按显式组织 slug 路由，业务事件只在事务成功提交后外发，不在 `notify()` 内执行 HTTP。群通知只发固定提示，摘要只发允许的统计字段和成员姓名（2026-09-18 起：最高 PCE 的负责人、昨日参与实验的同事及实验数），不发实验标题、内容或邮箱；没有生产凭据的测试不得触发真实群消息。
