...
### 17.3 release `20260826-006`（2026-08-26）

**内容**：`main` 推进到 `fe2436e`（PR #9）。无 migration。

**为什么发**：`Rose · 小面积 · batch 22` 这条队列项无论点哪个按钮都无法处理。核查后确认它
**不是重复数据**，而是被操作者表格拆开的同一批次的另一半：已发布的 `2026-001-20-22` 只有
样品 `D1`，卡住的这条带 `Me1`、`BrMe3`（各 10 个指标，3 个 J-V 文件），来源是同一个
`Rose/AI 数据统计-周翠翠.xlsx`。批次标题 对比三种不同的SAM分子 正好对应三个样品。

当时三个动作都不正确：Skip 会丢掉两个样品及其原始文件；Replace 会为同一 operator+batch
**再建一个实验**（EXPERIMENT 分支忽略 `updateTargetId`，永远走 `publishExperiment`）；
Remove 直接丢数据。另外重复检测只比对 title，因此对样品完全不重叠的两条记录报
`identical=true`，等于向审核人撒谎。两个缺陷都已在 PR #9 修复并有集成测试覆盖。

**发布前再次发现 APP_VERSION 漂移**：`current` 指向 `20260826-005`，而 `APP_VERSION` 停在
`20260826-004`。与 17.2 同一模式，说明这不是偶发。`pgrep` 确认当时没有并发发布；
`/proc/<MainPID>/cwd` 确认真正在跑的是 005。本次发布把标签修正为 `20260826-006`。

**数据前后对照**：

| 指标 | 合并前 | 合并后 |
| --- | --- | --- |
| `Experiment` 总数 | 954 | 954 |
| Rose 小面积-22 的实验数 | 1 | 1 |
| 该批次样品 | `D1` | `BrMe3, D1, Me1` |
| 该批次 characterization 数 | 1 | 1 |
| PENDING 队列 | 1 | 0 |

`D1` 原有的 10 个指标和 7 个文件未被改动；审计事件 `experiment.merged` 记录
`samplesAdded=["Me1","BrMe3"]`。合并通过正常的 ingest gate 执行，不是手工 SQL。

**验收**：`current` → `20260826-006`；`/api/health/live` 内网与公网都返回
`version: 20260826-006`。

**仍未做**：数据库服务器侧备份/异机副本/恢复演练依旧未完成，这仍是最高优先级欠账。

### 17.4 release `20260827-001`（2026-08-27）

**内容**：`main` 推进到 `b973072`（PR #13），LIGHTSKY 解析器修复。无 migration。

**触发**：大太阳 LSS-200 首次上线，第一份真实数据就暴露一个**静默数据损坏**缺陷。操作者对同
一组件反向扫了三次、正向一次，文件里因此有三条同名 `0820-15-8-Rev` 轨迹。解析器把汇总表建成
`Map<name, row>`，同名互相覆盖，三条反向曲线全部拿到最后一行的数值：

| 曲线块 | 真实 Voc / PCE | 入库为 |
| --- | --- | --- |
| 1 | 11.4514 V / 19.774 % | 11.0771 / 19.365（错） |
| 2 | 11.3547 V / 18.844 % | 11.0771 / 19.365（错） |
| 3 | 11.0771 V / 19.365 % | 正确 |
| 4（For） | 10.6675 V / 18.501 % | 正确 |

两条真实测量被第三条的副本顶替，自身数值被丢弃，全程无报错、无告警。

**配对不是猜的**：独立从每条曲线算出电流过零点电压，与对应汇总行的 Voc 精确吻合
（11.4514 / 11.3547 / 11.0771 / 10.6675，按块序），因此确定配对是**按文件顺序逐条对应**。
新测试断言的正是这个不变量，而不只是期望数字。原有 fixture 三条轨迹名字互不相同，所以从未
覆盖到冲突路径；已补一份同名 fixture（数字为虚构）。

**存量数据修复**：受影响的只有这一份上传的 4 行。用修复后的解析器重新解析 COS 中保存的原始
对象，按曲线特征（首点/末点/点数，四舍五入到 6 位——JSON 往返会改变末位，`141.7171` 存成
`141.71710000000002`）与库中行配对，先 dry run 再 `APPLY=1`，修复 2 行的 metrics /
measuredAt / settings / scanKey。修复后 4 行各自持有正确数值、4 个互不相同的 scanKey、
时间戳递增。这 4 行 status 都是 UNMATCHED，未挂到任何实验，因此不影响下游数据。

**第三次 APP_VERSION 漂移**：`current` 指向 `20260826-008`，而 `APP_VERSION` 停在
`20260826-006`。另外 `20260826-007` 和 `-008` 是在 8 月 27 日发布的，却沿用了 26 日的日期。
本次按手册用当天日期 `20260827-001`，并把标签修正。

**验收**：`current` → `20260827-001`；内网 `/api/health/live` 返回 `version: 20260827-001`；
两台仪器心跳正常（大太阳 02:42:06、小太阳 02:42:21 UTC），`lastError` 均为空。
